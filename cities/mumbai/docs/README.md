...
| | |
|---|---|
| Files in the manifest | **1,459** ([`data/MANIFEST.csv`](../data/MANIFEST.csv)) — 28,587 before the harvest rule of 21 September 2026 folded 13,500 per-response files into eleven archives (§9.201) |
| Catalogue | **600** sources in [`extract/sources.json`](../extract/sources.json), **572** acquired (22 of them dated Internet Archive copies of publishers that refuse this address, §9.208; 20 landed through an Indian VPN endpoint and the two OGD ridership series through the user's logged-in browser on 22 September 2026, §9.209, §9.210), **22** unobtained (each with the reason its host gave: a certificate that names another host or is out of date, a 404, a gateway time-out, a reset, or the OGD platform's logged-in download form), six acquired-unusable; eleven of them harvests (one archive of many public queries each). **Every entry has a data-use disposition** ([`source_use.json`](../data/processed/acquisition/source_use.json), `audit_source_use.py`): 253 consumed by a script, a transcription or a registry field, 121 discovery pages, 45 reference documents, 97 declared not needed with the reason, **0 unread** |
...
| Validation | **Thirteen per-mode targets** in [`mode_targets_by_mode.csv`](../data/processed/validation/mode_targets_by_mode.csv) ([`build_mode_targets.py`](../build/build_mode_targets.py), §9.205, §9.210): the shares derived from the CTS Updation 2017 MMR motorised split and its 47 % active share, the B-28 walk/bicycle split, the synthesised car-driver share, the MMB ferry passengers and the CMP goods share, each with its sweep — and, since 22 September 2026, the metro from the operators' observed daily passengers (MMRDA's OGD daily series for Lines 2A/7 and the Monorail, the Economic Survey's Line 1, 3 and Navi Mumbai averages: 2.56 % of the CTS's all-mode daily trips, the 2017 split gave 1.17 %); no projection to 2026, no holdout |
...
| light_rail (metro, monorail) | MMRCL stations, journeys and fares; metro fleet claims and per-line train capacities (§9.206); Navi Mumbai metro controls; MMMOCL's live first and last trains on Lines 2A, 7, 9 and 2B and MMRDA's April 2026 peak and off-peak headways (`A.baseline_transit.line_windows_s`, `line_headways_s`; generated departures within 8.3 % of the printed weekday trips, §9.209); MMRDA's daily ridership of Lines 2A/7 (1 January 2024 – 19 September 2025) and the Monorail (1 October 2024 – 21 September 2025) from the OGD platform ([`extract_ogd_metro_ridership.py`](../extract/extract_ogd_metro_ridership.py), §9.210), the metro target derived from them; MMRDA's 3.5 lakh daily passengers (March 2026) and Metro One's cumulative 1,101 million transcribed | the Monorail's service (the line runs in the feed on the assumed window); the seated/standing split of the BEML and Alstom trains; the Line 7–9 through corridor |
...
python cities/mumbai/extract/extract_ogd_metro_ridership.py
...
browser-captured HTTPS response against the catalogue before importing it, and
imports a file the user downloaded from a logged-in portal
(`--portal-download FILE --id ID --portal-page URL`, §9.210) with the portal
page and the file's modification time as its provenance.
