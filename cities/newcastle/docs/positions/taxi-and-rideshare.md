...
- **Fares** (`cities/newcastle/registry/B_demand.json`): taxi `B.taxi.flagfall_taxi_aud` 5.00 and `B.taxi.fare_per_km_taxi_aud` 2.52, both `measured` from the Point to Point Transport (Fares) Order 2025 urban schedule archived at `data/raw/p2p/` (§9.76); rideshare `B.taxi.flagfall_rideshare_aud` 1.95 and `B.taxi.fare_per_km_rideshare_aud` 1.50, `literature`, swept (§9.76). Surge, night rates, the peak surcharge and the passenger service levy are recorded and deliberately not charged (§9.76).
...
- **The median modelled taxi trip is 13,072 m**, which contradicts the premise of the held-fixed rule on `B.taxi.fare_per_km_taxi_aud` (§9.91); see Refused.
