...
- **Registry and checks.** 457 fields (`docs/reference/CONFIG_REFERENCE.md`, §9.140), `check_hardcoding.py --strict` at 0 and gating CI; every `assumed`/`literature`/`measured`/`derived` field carries a sweep, a `held_fixed` rule or a `derived_from` identity, and the three unobtained fields hold `value: null` so `get()` raises (§15). No key name carries a currency: the 29 `A.fare`, `A.parking` and `B.taxi` keys dropped their `_aud` suffix and the fare config parameters their `Aud` suffix on 3 Sep 2026; the currency lives in `units` and `city.json` (§9.140, #62). 511 files in `data/MANIFEST.csv` (§9.140); `check_manifest.py` in CI, `check_package.py` local over city-owned expectations (#62), `check_doc_currency.py --strict` pinning every live-state figure in `README.md` and `STATUS.md` (§9.79).
