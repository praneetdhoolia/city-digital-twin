...
- **A shared pair shares a sampling-hash bucket the width of the campaign fraction.** `B.ride.shared_lift_hash_bucket` **0.25** since §9.149 (`assumed`, sweep [0.05, 0.25]; 0.05 was §9.129's value and is the control): the fourth binder pass (`B.ride.shared_lift_scope` = `same_sa2_od`) binds a passenger only to drivers whose household hash sits in the same bucket, so every nested sample at a multiple of the width keeps both members, and a bucket prefers no hash (§9.129); it replaced the §9.127 at-or-below rule. At 0.05 each passenger could be served by 5 % of drivers, which only the short intra-suburb trips survived — 94 % of the 27,771 unserved car-less tours, median 9–15 km, had a driver in the window and were refused on the bucket alone (§9.149). A 1 % smoke breaks pairs as it always did and never read pairing (§9.128). The binder records the seed it hashed under; a run under another `RUN.machine.seed` breaks the guarantee and the report names the seed that holds it (§9.127).
...
| `F28-the-car-waits-only-for-a-car` | 20260907T025531 | the F27 stack with the household car constraint enforced car-only in `HouseholdCarDepartureHandler` and `RUN.qsim.vehicle_behavior` back to `teleport`: F27's one arm ran under a global `wait`, which stranded walk and taxi (iteration-0 car departures 82,388 against F26's 232,394, 55,862 car agents stuck) and was stopped at 19, citable for nothing. The agent source inserts vehicles before the agent. Demand, roster, listener scope, carve, cadence and threads are F27's. **Opened at the FIX** | §9.148 |
| `F29-lifts-are-the-long-trips` | 20260907T114503 | the F28 run stack on a demand where the shared-ride pass may draw drivers from the whole 25 % sample — `B.ride.shared_lift_hash_bucket` 0.05 → 0.25, the standing campaign fraction — and, thinning for the first time on the weekday (`thin_p` 0.9926 → 0.7517), binds the longest servable tours first (`B.ride.shared_lift_priority` = `longest_first`): its bound trips' median 2.46 → 5.79 km, mean 4.48 → 8.14 km. At the F28 gate car was inside (+6.6 %), pairing solved and the roster live, and 39 % of the pass's lifts were walked at a median 1.08 km. Chains, plans and the 30 sets rebuilt; run stack F28's. **Opened at the REBUILD, not at a launch** | §9.149 |
| `F30-an-escort-is-priced-as-an-escort` | 20260907T144147 | the F29 demand with **one** change that moves a result: `EscortCoherenceListener` iterates a `TreeMap`, so its seeded draw follows the household id instead of HashMap order — the order the sample fraction changes, which made a 1 % probe unable to predict a 25 % arm on that path (§9.151, #150). The HTS purpose map was made one at the same rebuild (`Serve passenger` → HX everywhere, #147) and the VOT table re-keyed `NHB` → `HX` at the same 15.2: **the 30 emitted configs came back byte-identical**, so that half moved nothing and the boundary rests on the draw order alone. Demand, run stack and network are F29's. **Opened at the REBUILD, not at a launch** | §9.151 |
| `F31-the-car-router-reads-only-cars` | 20260908T095937 | the F30 demand and network with the travel-time table fed by **car alone**. `RUN.travel_time.filter_modes` was declared true in §9.154 (#154) and this is the first arm to carry it: MATSim's default is false, and with it false `analyzedModes` is read by no handler, so every vehicle on the network fed the ONE table that `RUN.travel_time.separate_modes` = false creates and that MATSim then binds as the observed travel time for EVERY network mode — a pedestrian at 1.4 m/s, a cyclist, a bus that stopped on the link and a truck at its cap, all averaged into the seconds the CAR router believed a link cost. Route choice changes wherever a walked or ridden link had been inflating the car estimate. The compiled controler also changed and a recompile is a boundary by the rule (§9.127, §9.138): `NetworkDirectWalkPtRouter`'s counters became run-lifetime atomics (§9.156), which is LOGGING ONLY and alters no routing decision, but it is recorded here rather than argued away. **Opened at the ARM**, and its first arm is `20260908T100009_300it_25pct` | §9.154, §9.156 |
