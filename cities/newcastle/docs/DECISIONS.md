...
**Since 30 August 2026 (§9.132) this file is the frozen record, entered through
the position pages.** The current truth for a topic is in
[`positions/`](positions) (one page each, every figure sourced); the board is
[`STATUS.md`](STATUS.md); the goal is [`GOAL.md`](GOAL.md). Read a section here
for *why*, never the file for *what is true now*, and never the file whole:
`grep -n "^## 9\.NNN"` then `sed -n`. Sections through §9.131 are frozen. A new
section is appended after the last `## 9.x` (before `## 14.`), numbered next,
at most 140 lines, on the template in the `/handoff` skill, with a row in the
index below and a row in §14; a correction to an earlier section is an edit to
the position page plus a §14 row, never a new section (`tests/check_doc_shape.py`
enforces the order, the cap and the index row).

This file is **long and append-only** — over thirteen thousand lines. Two things
about its layout will otherwise cost you an hour:
...
| Looking for | Read |
|---|---|
| **START HERE — the goal, the board, the current truth per topic** | [`GOAL.md`](GOAL.md) · [`STATUS.md`](STATUS.md) · [`positions/`](positions); how this record is now kept: **§9.132** |
...
| **Non-household lifts (the reported gap, now mechanised)** | **§9.60** — M0 physical waiting at the meeting point; M1 re-targets unbound observed-rate escort tours to driverless-household passengers; pairing/boarding/sampling integrity; dossier [`design/non-household-lifts.md`](archived/design/non-household-lifts.md) |
...
| **Bike stress, parking search time, income: three literature channels enter** | **§9.138** — from the two research artifacts, on user directive: `bike_stress_factor` per link (Broach et al. 2012 felt-distance factors, swept) charged in score and router; a priced zone's derived search minutes (Shoup 2006, scaled by the §9.31 density ramp) charged once per parking spell; each resident's G17 band midpoint scales marginalUtilityOfMoney (MATSim core, exponent swept). Crowding deferred until fares settle rail (#98). Family F23 at the next launch |
...
| **The specification audit** | §9.25 and [`docs/archived/audit/SPEC_AUDIT.md`](archived/audit/SPEC_AUDIT.md) |
...
> **Frozen snapshot of 10 August 2026.** The table below was true at P1 and is
> kept as written. The current position on every input is
> [`positions/network-and-inputs.md`](positions/network-and-inputs.md); the
> paragraph beneath the table — three inputs "handled by sweep" — is superseded
> by `GOAL.md` requirement 6 and by §9.88 (SCATS implemented), §9.130 (rail and
> tram on disclosed boardings) and §9.131 (licence rates measured).
...
constant. The full ranked register is [`docs/archived/audit/SPEC_AUDIT.md`](archived/audit/SPEC_AUDIT.md);
...
with two 2 a.m.s ([`docs/archived/audit/ISSUE_VERDICTS.md`](archived/audit/ISSUE_VERDICTS.md)).
...
convergence, in
[`docs/archived/audit/CONVERGENCE_PILOT_EVALUATION.md`](archived/audit/CONVERGENCE_PILOT_EVALUATION.md).
...
[`docs/archived/design/age-structure.md`](archived/design/age-structure.md). Three defects, all
...
[`docs/archived/design/mode-individualisation.md`](archived/design/mode-individualisation.md).
...
dossier** ([`design/physical-ride.md`](archived/design/physical-ride.md)) — chosen by
...
[`design/non-household-lifts.md`](archived/design/non-household-lifts.md); the
...
§9.50: ASCs stay priors. `docs/reference/CALIBRATION_REPORT.md` regenerated.
...
[`design/signalling/`](archived/design/signalling/README.md) (moved from a temporary
...
- **Cross-run index (4.7.3, #77).** `build_run_index.py` → `results/INDEX.md`
  ...
  `docs/run_families.json` (F1 pilot → F5 ride/walk repairs, each with
...
- The 30 run-input sets regenerated; the boundary is ONE boundary, and
  **family F6 is declared in `docs/run_families.json` in this change.**
...
`docs/archived/audit/CORRIDOR_PT_COMPOSITION.md`; re-measures on the first F6 arm.

**The demographic measurement (#50) is on the record** (`docs/archived/audit/
...
- **It draws the calibrated base's own run.** Not the newest directory (usually a
  ...
  `docs/reference/CALIBRATION_REPORT.md` always describe the same arm and both follow
...
`docs/archived/audit/CORRIDOR_PT_COMPOSITION.md` and through three consecutive handover
...
[`run_families.json`](run_families.json). F10 has no completed arm to
...
## 9.132 The project on one page: the goal stated, the board generated, the record frozen behind position pages (30 August 2026, seventeenth session)

**What was wrong.** Measured on `main` at `3e5b036`: this record was 911 KB and
13,194 lines with 122 `## 9.x` sections, 1,845 and 1,909 lines added on 29 and
30 August alone, seventeen `CORRECTION` markers and §9.127–§9.129 in reverse
order in the file. The board was 116 KB and 805 lines with a 2,923-character
*Last updated* cell and eleven narrative sections under a line-9 rule that said
*not a diary*. The brief was 406 lines and named F15, F16, F17, F18 and F20 as
the running family in different sections, because it had been patched across
five family boundaries in one day instead of rewritten. The operating goal — the
user's `/goal` directive of 24 August — existed only as an untracked `GOAL.md`
and as "the session's `/goal`" quoted eleven times here, while
`.claude/CLAUDE.md:8`, `README.md:3` and the proposal defined the project as the
light-rail counterfactual; in the record from §9.80 on, the proposal's hypotheses
are mentioned three times and ridership twenty. Four living documents still said
"SCATS phasing is unobtained, handled by sweep" two days after §9.88 implemented
the algorithm, and `CLAUDE.md:30` said "No scenario has been run" after ~118 run
directories existed — neither sentence was in the scope of the currency check,
which pinned 32 numbers in two documents. `/onboard` pointed a fresh session at
the record, the board and the brief whole: about forty minutes of reading before
a briefing.

**What changed.** No model value, no data artefact, no family boundary.

- `cities/newcastle/docs/GOAL.md` is tracked: the eleven hard requirements, the
  gate loop and the monitoring rule as the user stated them on 24 and 30 August,
  with what they supersede (the proposal's framing and its §7.2 "no SCATS →
  sweep" fallback). `CLAUDE.md` and `README.md` define the project by it.
- `STATUS.md` is one page: a goal table, three **generated** blocks
  (`src/analyse/build_status_board.py` — the twelve-mode scoreboard from the
  newest readable run, the state from the ledger, registry, manifest and
  run-input sets, the runs on disk with their causes), a phase table, *Next*,
  *Open work* and *Do not re-raise*. Its hand-written lines are capped at 170.
  The retired body — batches 4.1–4.15, the delivery tables, the run-cost
  history, the narrative sections — is archived verbatim in
  `archived/SESSION_LOG.md`.
- Thirteen **position pages** under `docs/positions/` state the current truth
  per topic (ride and pairing; sampling and families; seed and choice set;
  signals and crossings; taxi; walk and bike; public transport and yardsticks;
  motorbike, truck and freight; population and demand; runs and economics;
  monitoring and gates; light rail and ferry; network and inputs), each under
  130 lines, every figure with its source on its line. They were distilled from
  this record in parallel and reviewed; the contradictions they surfaced are
  recorded on the pages as open items, not resolved here.
- `report_mode_ridership.py --trend` prints all twelve modes (freight rail was
  absent) with a basis column, formats boardings as integers, and writes `--json`
  for the board.
- `tests/check_doc_shape.py` (CI, rules in `cities/newcastle/tests/doc_shape.json`)
  caps the board and the brief, requires the brief's family stamp to equal the
  ledger's newest key, requires new sections here to be appended in ascending
  order under 140 lines with an index row, requires a source on every figure of
  a position page, and requires a banner on every frozen document. Thirteen
  documents under `audit/`, `design/` and `handover/` are bannered as frozen,
  the proposal among them. `check_doc_currency.py` now covers `CLAUDE.md`,
  `docs/README.md`, the brief and `GOAL.md` (45 claims), banning every phrasing
  of the four false statements above.
- `src/run/session_gate.py` is the one gate both skills run (it skips the
  toolchain compile while an arm runs — this session ran the old gate under the
  F20 arm, which the brief's first trap forbade); `--digest` prints the goal,
  the board's blocks and the machine state at SessionStart. A `PreToolUse` hook
  refuses `gh pr create` while either document gate is red. CI gains the shape
  job and the board-block check.
- `docs/HANDOVER_CONTRACT.md`, `/onboard` and `/handoff` are rewritten: a
  600-line reading budget, the trust order by question, four questions instead
  of six, a *Consolidate* phase that rewrites the touched position pages, the
  section template above, the brief template (four sections, 180 lines).
- Issues #73 and #68 closed on the evidence the position pages cite (§9.88,
  §9.90); #21 commented, not closed — the gradient physics channel is built
  (§9.84) but the `C.gradient.*` scoring weights and the PT walk-access decay
  still reach nothing.
- At the user's direction the frozen documents moved under `docs/archived/`
  (`audit/`, `design/` with the signalling dossier, the two old briefs and the
  session log), the ledger `run_families.json` and the brief moved to the docs
  root, and the generated `CALIBRATION_REPORT.md` to `reference/`, so `docs/`
  holds only what is live. 71 markdown links were re-resolved from each file's
  new location, this record's among them — link maintenance, not a rewrite;
  `layers.json` was regenerated for the two moved code-read paths.

**Measured.** Board: 805 → under 170 hand-written lines plus generated blocks.
Brief: 406 → under 180 lines. Reading to a briefing: the record, the board and
the brief whole → about 600 lines (digest, `GOAL.md`, board, brief, one
position page). Currency claims 32 → 45; shape rules 5 classes; frozen documents
bannered 13. The scoreboard reads the F20 arm at iteration 10: one mode inside
10 % (motorbike), eight past the 20 % bar, truck on a level only, freight rail a
representation — the same numbers the sixteenth session's brief carried, now
generated rather than typed.

**Deliberately not done.** The demand chain on the F21 population was not
rebuilt and the F21 arm was not launched: that is the model lane, it changes
bulk data and the manifest, and it needs a stated-cost approval. This record's
existing sections were not reordered, renumbered or edited — §9.127–§9.131 stay
where they are, out of order, and the shape rule applies only above §9.131. The
S2 tram-priority ambiguity the signals page found (the emitted config says
`green_extension`; the record's S2 probe ran with it off) is recorded as a
decision required, not decided. Position pages were not written for the
scenarios S0–S6 or the calibration loop, which have had no session since the
goal changed; the record's §10–§12 stand.

**Consequences.** A family now costs one ledger row, one section here and one
position edit; the board and the brief are regenerated or rewritten, so cadence
no longer makes documents wrong. This file is frozen through §9.131 and grows
only by the template. The four false statements cannot return without failing
...
Registry 452 → 462. **Nothing here is a result**: no arm has run on this
boundary, and the channels' effect is a prediction until its gate reads.
...
| 2026-08-30 | **Seven in ten refused joint bindings name the companion as their own driver (§9.111; issues #86/#91/#48).** The binder now classifies its refusals; run for WEEKDAY with approval and **verified byte-identical against the committed demand before any number was read**, so no family boundary was opened. Of 73,258: `driver_is_the_companion` **51,215 (69.9%)**, shift-pass driver committed 12,530, as-timed collision 9,511, and **`driver_party_full` just 2** - vindicating the claim written on `B.ride.max_passengers_per_vehicle` that it does not bind, and withdrawing the suspicion held against it while the clause was lumped. Driver and companion pools are built from the SAME tours, so a licensed car-owner sits in both; **41.7% of multi-person households hold at most one licensed travelling member**, and that person drives relatives but can never be served. `p_thin` therefore thins a pool a third of which is unbindable by construction. Fix identified - exclude companions whose household holds no OTHER eligible driver tour - changing no parameter, observation or target, but it is a FAMILY BOUNDARY and is left for a deliberate rebuild. Nothing here is a finding. |
| 2026-08-30 | **CORRECTION to §9.109: the companions' days are not full (§9.110; issues #86/#91).** §9.109 said the 73,258 refused joint bindings fail because the companion's day is already full; that was read off the binder's source comment, not measured, and the measurement contradicts it. Committed time per person against the 30 h horizon: **median 6.25 h (20.8%)**, and only **4.6%** commit more than half. Driver tours are not scarce in aggregate either (~200,000 commitments against 1,100,353 tours), so any shortage is household-local. §9.109's engine-side numbers stand unchanged. The binder gained a five-clause refusal classification - counters only, 19 insertions, no control flow, no draw, no ordering, no output row. Nothing here is a finding. |
| 2026-08-30 | **The demand generates 42% more ride legs than it generates drivers for (§9.109; issues #86/#91/#48).** Measured at **iteration 0**, before a single replanning pass: 44,750 ride legs generated, **24,833 pairable (55.5%)**, 13,176 whose household drove elsewhere at ANY hour, 5,787 with no household car leg at all - **42.4% unservable by the demand itself**, and the endpoint class is 66.2% of failures at iteration 0 against 66.0% at iteration 100, which is structural rather than emergent. That closes what §9.102 opened: the pairing rule, the bound window (§9.98) and the coherence rate (§9.93) all measured real-but-marginal because there was nothing for them to find. The binder's own build-time report agrees from the other end: escort binding 127,203 of 177,318 tours (71.7%), joint binding reaching 176,969 of its own 448,229-trip target (**39.5%**) with **73,258 skipped as infeasible against 74,663 bound**. Nothing here is a finding. |
| 2026-08-30 | **Read as a trend, car walk and pt were converging all along (§9.108; issues #48/#49/#50/#30).** Every gate this session was read at iteration 100 of a 1000-iteration arm whose innovation runs to 800. On the trend: **car 0.3409 → 0.4429 (+0.001019/it, ~136 more iterations), walk 0.2888 → 0.2116 (-0.000772/it, ~100 more), pt 0.0688 → 0.0603 (~248 more)** - all converging, arriving between iterations 200 and 350; and walk's GEOMETRY converges with its share (21,475 trips → 15,955, mean 8.12 → 6.66 km). **Genuinely diverging: ride, bike, motorbike.** The yardstick repairs (§9.100, §9.101) are untouched by this - those are wrong at every iteration. **A gate that stops a run before the model can answer turns every transient into a defect**, and this session chased four. No arm in F11, F12 or F13 has ever reached its innovation cutoff. Nothing here is a finding. |
| 2026-08-30 | **CORRECTION: the demand is not the defect - walk and car are swapped at both ends (§9.107; issues #30/#49/#50).** §9.103 and §9.106 both concluded the walk defect lay in destination placement; both reasoned rather than measured, and the anchor existed. The HTS publishes **DISTANCE_BY_MODE** beside the mode shares, carried into `hts_mode.csv` at acquisition and never used: for the target LGA, resident trips **1.11x** observed, total km **1.20x**, mean trip length **1.08x** (9.87 km against 9.14), and **16.31% of modelled trips are under 1 km** against an observed walk-only 13.4%. The demand has the short trips. The defect is ALLOCATION: of 10,247 sub-1 km trips only **39.5% are walked while car takes 39.2%**, and walk's resident mean is **6.66 km against an observed 0.70**. Modes are assigned per SUBTOUR, so a car chain drives its 800 m leg and a walk chain walks its 20 km leg. `mode_targets_by_mode.csv` gains `target_mean_km` from the survey's own TRIP_AVG_DISTANCE and the gate now prints geometry beside every share. No target share changed. Nothing here is a finding. |
| 2026-08-30 | **A feasibility bound on walk does not work, and the reason says where the defect is not (§9.106; issues #30/#49).** Walk is the only mode nothing constrains, so a derived bound was built - the p99 of an exponential with the declared observed 0.70 km mean, **3.22 km**, corroborated to within a kilometre by the observed mean walk TIME through this model's own speed cap; bike 23.95 km. It failed twice. First it killed the run: refusing one trip inside a subtour leaves it mixing chain- and non-chain-based modes (`IllegalStateException`, rc=1 at once), so a refusal must reject the WHOLE proposal. Then, chain-safe, it made the fit worse - the sum of absolute deviation over the seven scored modes went **509.9% → 577.1%**, taxi blew out **+15.2% → +135.1%**, and walk's mean moved only 8.84 → 8.72 km. **A choice-set gate in the replanner can only refuse NEW proposals; it cannot remove behaviour already in the plan**, and the forced whole-proposal rejection can trap an agent. Both bounds set to **0.0, disabled and reproducing**; the mechanism stays declared and swept. +2 registry fields, both derived. Nothing here is a finding. |
| 2026-08-30 | **A denied lift becomes a drive (§9.105; issues #48/#30).** Reading walk's GEOMETRY rather than its share: mean trip **8.75 km against a declared observed 0.70** - 12.5x - with 46% of walk trips over 5 km and the longest 91.4 km, while ride over the same trips is 0.97x its observed mean. **16,153 unpaired ride legs and 1,602 refused taxi requests are forced into walk in ONE iteration - 31% of all walk trips.** Neither fallback was ever argued for as behaviour. `B.ride.unpaired_fallback` (+1 field, swept, `walk` reproducing) now executes the leg as CAR when the passenger holds a licence and a car, leaving everyone who cannot drive walking; the ride alternative is restored at AfterMobsim under both members, which needed the restore matcher generalised. Measured on a committed pair: six of seven modes moved toward target and **taxi came inside the bar at +7.5%**, but **pt moved AWAY (-19.6% → -30.2%)** and **walk's geometry barely moved (9.30 → 8.80 km)**. Declared risk: walk was high and car low, so this flatters both - which is why it ships measured against a reproducing control. Nothing here is a finding. |
| 2026-08-30 | **Resume matched two runs differing in a declared value (§9.104; issue #66).** The §9.102 pair was launched as two committed overlays differing in ONE field, and the test was handed **the control's completed run**: `find_completed` compared scenario, day, fraction, iterations, seed, `--set` overrides and the controler hash but **not the resolved registry values**, which is exactly where a run-config overlay sets things, and the Java was identical so even the controler hash matched. A paired diagnostic would have reported a difference of zero. `_run.json` now carries **`values_sha256`** over every resolved registry value and `find_completed` requires it; a record written before this carries no hash and does not match, by design, so completed runs predating the change must be re-run rather than resumed. Nothing here is a finding. |
| 2026-08-30 | **The light rail is not out-competed, it is out of reach (§9.103; issues #30/#49/#84).** The whole within-PT error is the intervention: **148 boardings of 21,429, -95.9%** of its observed share. Not supply, and not mainly mode choice: **only 2,350 of 221,144 trips (1.063%) have BOTH ends within 800 m of a stop**, and of those 83 choose PT at all and 28 the light rail, against car 1,010 and walk 928. A tram corridor through the CBD holding 1.06% of trips is a statement about DESTINATION PLACEMENT, which lands on #30. **The corridor market is a MODELLED quantity and may never be used to argue the observation down** - that would be §9.92's error in a new costume. (Its "550 trips a day" figure is corrected to 252 weekday departures by §9.113; the conclusion stands.) Nothing here is a finding. |
| 2026-08-30 | **The pairing rule that can represent a drop-off buys nothing (§9.102; issues #48/#91).** `both_links` requires the driver's leg to START and END on the passenger's own two links, so a parent driving a child to school on the way to work is refused by construction however wide the window opens - and **64.4% of pairing failures are that class**. `route_contains` was built: the passenger's links must lie on the driver's routed path in order, with the driver's time apportioned by the carried segment's share of route length, unity when the segment is the whole route so `both_links` reproduces exactly. Measured on a committed pair differing in this field alone: pair rate **0.5069 → 0.5113**, `miss_endpoints` **1,257 → 1,233**, and ride mode share went the WRONG way, **9.6748 → 9.4513**. The driver's PLAN carries no escort stop, so the driver's PATH does not pass the passenger either. **§9.92's ruling that `both_links` stays is CONFIRMED**, now by measurement rather than the circular argument it was made on; `route_contains` stays a declared sweep member. Third pairing-side lever measured real-but-marginal. Nothing here is a finding. |
