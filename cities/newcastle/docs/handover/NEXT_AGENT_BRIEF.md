...
|---|---|
| **AN F20 ARM IS RUNNING — THE MACHINE IS NOT FREE.** `results/20260830T184955_300it_10pct` (overlay `f20_gate_10pct`: S2, WEEKDAY, 10%, 300 it, cutoff 240), launched 30 Aug 18:49 as Task Scheduler task `citysim_run_20260830T184954`; demand re-bound under the same-bucket rule, carves solved on the drawn pool (§9.129), driver detour (§9.128); first gate at iteration 100 (~23:50). The F19 arm `aborted_20260830T170743_300it_10pct` was stopped at iteration 27 (biased sample composition, §9.129). | `Get-ScheduledTask -TaskName citysim_run_20260830T184954` · `tasklist | findstr java` · `tail -2 results/20260830T184955_300it_10pct/matsim.log` |
| Which iterations it has written, and its per-mode reading at each | `python src/analyse/report_mode_ridership.py --run results/20260830T184955_300it_10pct --trend` (every readable iteration, every mode, direction) · `--it <n>` for one table · `--watch 300` to keep printing — every 10th iteration is readable now, not only the trips-table ones |
