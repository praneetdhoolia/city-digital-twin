...
**Last updated:** 30 August 2026, SIXTEENTH session, at handoff (**§9.127-§9.131**: the population a run sampled from is part of its identity; a declared ride pair whose links differ is served by the driver's detour (F19); the 9.127 coupling rule biased every sub-sample and the carves were solved on a pool that is not drawn (F20 - the carve fix read motorbike at -0.1% at iteration 10); heavy rail and light rail are held to their DISCLOSED boardings; and a seventh of the workforce held no licence - the licence rate is now the TfNSW count over the ABS population, per LGA, and the population is rebuilt with it. **The machine is idle; the package on disk is INCONSISTENT** (population newer than chains) until the demand chain is rerun - the next session's first build.)
