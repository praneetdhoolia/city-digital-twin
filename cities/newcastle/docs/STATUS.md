...
| Blocking state | **THE ITERATION-200 GATE IS THE LOOP, AND IT HAS FIRED TWICE (§9.81, §9.82).** No approval is outstanding: the session's `/goal` authorises stop, diagnose, fix, relaunch, and the F8 arm `20260826T222352` is RUNNING under it. Two causes are found and repaired, neither by moving a parameter: the **ride ratchet** (§9.81 - a missed pairing was deleting the alternative from the plan) and the **empty escort tours** (§9.82 - 84.53% of escort trips are car while 11.45% of escort-bound members ride, a pair per-agent replanning cannot rejoin). **What is NOT yet answered is whether either moves realised ridership**: F7 lifted realised ride only 1.3% to 1.7% and left car, bike, taxi and pt within 0.15 pp of the unfixed arm. `bike` (9.97 realised vs 3.2 observed) and `taxi` (8.36, about 7x its declared 15,000-25,000/day band) remain over-chosen and UNDIAGNOSED - the standing candidates are the absent age gate on independent bicycle and taxi use (`age` is written as a person attribute and consulted by NOTHING; `AvailabilityModesCalculator` gates rideAvail, bikeAvail and lockedMode, and taxi not at all), which needs CWANZ age-specific participation that is OUTSIDE the sandbox allowlist. `E.replication.n_replications` and the warm-restart ruling stay open |
...
| 4.11.9 | **Run F8 and gate it at iteration 200** | — | **RUNNING** — `20260826T222352`, launched 26 Aug 22:23 |
| 4.11.10 | `bike` and `taxi` over-choice | #49 #50 | **UNDIAGNOSED** — `age` is written as a person attribute and consulted by nothing; taxi has no availability gate at all. CWANZ age-specific participation is outside the sandbox allowlist |
