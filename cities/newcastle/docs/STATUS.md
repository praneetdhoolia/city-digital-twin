...
**Last updated:** 29 August 2026, THIRTEENTH session (**§9.86–§9.91**: THE GROUNDWORK STOPS BEING ASSUMED, AND THE FIRST DEFECT THE GATE FOUND WAS IN THE YARDSTICK. The `/goal` directive was amended mid-session to forbid leaving an unavailable input SWEPT where it can be DERIVED. Four things were derived or made physical: **taxi stops being teleported** (§9.86 — it was network-routed, link-permitted, congestion-bound and car-bodied but absent from `RUN.qsim.main_mode`, so 39,892 of 39,923 legs an iteration never touched the carriageway; family **F11**); **twelve modes get twelve individual targets** (§9.87 — the HTS publishes SIX categories, so bus/light rail/heavy rail/ferry shared ONE 3.8% row); **SCATS becomes an implemented ALGORITHM** rather than a fixed 110 s plan (§9.88 — degree of saturation measured at every signalised stop line, incremental cycle adaptation, splits equalising DS; offsets deliberately NOT adapted; family **F12**); **the ferry gets a derived target** (§9.89) and **level-crossing closures are derived from the mapped rail timetable** — Adamstown 110/weekday and Islington 204 against an assumed flat 30 (§9.90). Then the F12 arm `20260828T232428` was stopped at the iteration-50 gate with **ten of twelve modes past 20%**, only heavy rail inside it. **§9.91: the first defect found was the TARGET, not the model** — §9.87 had sized taxi from the census JOURNEY-TO-WORK share, and taxi is overwhelmingly a non-commute mode, so the target was fivefold low; `B.taxi.daily_trips_band` (IPART 2025) gives 0.9916% and bike takes the residual 2.2084%. A paired 1% diagnostic then proved the per-km fare BINDS elevenfold, killing the mispricing hypothesis, and a reader bug was found that counted a linked PT trip once per submode boarded — which had been hiding a light-rail and ferry deficit. **Two intermediate readings in that investigation were wrong the same way: read off a curve that was still moving.** NOTHING IS A RESULT: no arm in F11 or F12 carries a `_run.json`.)
