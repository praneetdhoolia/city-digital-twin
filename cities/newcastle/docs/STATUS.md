...
| Blocking state | **The relaunch is APPROVED AND IN FLIGHT (§9.62, 21 Aug third session)**: two concurrent arms under the §9.59 pattern (qsim 8 + events 4 + xmx 30g each) — **arm A `phys1000a_25pct`** (S2 × WEEKDAY, 25% × 1000, master seed; the base arm C5 and every re-baseline reads) and **arm B `phys1000b_25pct`** (identical, seed 20260811; the `E.replication.n_replications` seed-variance measurement). The approval is consumed — none is standing after these arms. The earlier first attempt was STOPPED at owner instruction during iteration 136 (21 Aug ~11:52), quarantined to `results/_aborted_20260821/phys1000_25pct` — no `_run.json`, not a result. **The #60 decide-before-relaunch question is RESOLVED (§9.58)**: the defect was verified to be a different mechanism than filed (activities off the walk/bike subnetworks + MATSim's silent nearest-link route fallback, not turn restrictions), and the four repairs are BUILT and probe-verified — refusals 3.8k/iteration → **0** at 1%. §9.58–§9.61 start a NEW family (network + model + demand change, free — no completed run existed). The arm would run at a measured **~233 s/iteration → ~65 h single, or two concurrent arms (qsim 8 + events 4 each, §9.59)**; relaunch requires fresh stated-cost approval. It re-baselines everything at once: the emergent ride share (now with §9.60 lifts) vs 20.60, walk vs 13.40 (#30's numbers are from the wedged model and re-measure here), the §9.50 constrained base (C5 + report). #54's accounting fix is MERGED (PR #58), so the run's own gate is trustworthy |
...
| Ride scenarios — data grade | **Commute carpooling is RARE and the demand is non-commute**: census G62 (already in the package) gives car-as-passenger **3.35% of journeys to work**, passenger:driver **0.0598**, at SA1 — against an all-purpose HTS `Vehicle passenger` share of 18–32%. OBSERVED: commute (G62), driver-side `Serve passenger` 10–19.5% of journeys, all-purpose share, ride trip length/duration, occupancy 0.35. LITERATURE ONLY: child→school (61% of school trips by private vehicle), elderly driven. **Non-household lifts now have a MECHANISM but still no target** (§9.60, owner-directed): unbound observed-rate escort tours are re-targeted to driverless-household passengers — WEEKDAY binds 55,280 of 55,614 (99.4%) — and a booked passenger physically waits for the car (M0); who-drives-whom stays unobserved, so the household/non-household split is reported, never fitted. Return-trip asymmetry remains a stated limitation |
| Comparability | **A third family boundary at §9.58/§9.59 (21 Aug)**: the walk-wedge repairs change the network (reverse walk/bike complements, trunk walkable, activity links pinned) and the model (SubtourModeChoice person-only, PassingQ link dynamics, split replanning threads), so **nothing run after §9.58 compares to `phys50_25pct`, the aborted `phys1000_25pct` diagnostics, or anything older**. The §9.49 boundary before it: freight changed the demand AND the model, so `bind1000_25pct` closed the §9.46/§9.47 family. The earlier triple break (§9.44/§9.45/§9.46-47) stands; the two pilot arms remain baselines for the PRE-repair model only. §9.43 (iterations=1000) is unaffected |
| Runs on disk | `smoke_postrebuild` (plumbing, 1% × 2) · **`conv1000_10pct`** and **`conv1000_25pct`** (the #5 pilot arms, both rc=0, both **`relaxed: true`** — evaluation in [`docs/audit/CONVERGENCE_PILOT_EVALUATION.md`](audit/CONVERGENCE_PILOT_EVALUATION.md)). `results/_aborted_20260816/` quarantines three dead runs without `_run.json`, including the cancelled `conv1500_10pct` — **do not relaunch it**. ⚠️ `results/S2_WEEKDAY_f025_i1000_s20260810/` is a fourth dead run with no `_run.json`, not yet quarantined. `ride_pairing_probe` (§9.44), `rp25_stress` (the §9.44-era 25% × 10 pairing stress probe, rc=0), **`freight_smoke`** (§9.49), **`motorbike_smoke`** (§9.52), **`jointride_probe`** (§9.53) and **`allmodes_probe`** (§9.54/§9.55 — the all-physical model runs end-to-end) are PLUMBING PROBES, not results. `results/_aborted_20260818/bind1000_25pct` is the STOPPED first attempt at the re-measure arm. **`results/_aborted_20260820/` quarantines two dead runs**: `base1000_25pct` (the §9.50 base arm, stopped by the owner at ~iteration 20 — relaunch needs approval) and `S2_WEEKDAY_f025_i1000_s20260810` (the old unquarantined dead run, now filed). **`results/bind1000_25pct/` is the completed re-measure arm** — rc=0, `relaxed: true`, evaluated in §9.48 — and is **the LAST run of the §9.46/§9.47 family: nothing run on the §9.49 freight demand compares to it**. **21 Aug additions**: `phys50_25pct` (the all-physical 50 × 25% shakedown — rc=0, accounting closes on every mode under the events-based gate, median 261.1 s/iteration measured; a shakedown, not a result), `evthreads_ab` + `evthreads_ab2` + `evthreads_timing` (the §9.56 bit-identity and timing probes), and **`results/_aborted_20260821/phys1000_25pct` — the first attempt at the converged all-physical arm, STOPPED at owner instruction during iteration 136** (no `_run.json`, not a result; 135 iterations of trajectory diagnostics preserved — car had reached 46.3% and walk fallen to 32.8% at it 133; relaunch needs fresh approval). **Session-close additions (all PLUMBING/TIMING, none a result)**: `wedge_probe`/`wedge_probe2` (§9.58 verification, refusals 3.8k/it → 0), `phys_timing2_base`/`_evt`/`_async`/`_fifo` (the §9.59 knob probes, 25% × 5 each), `lift_probe` (§9.60 verification on the regenerated demand — rc=0, 0 refusals, waiting counters live, accounting closes). **IN FLIGHT (§9.62, 21 Aug third session)**: `phys1000a_25pct` (arm A, the base arm) and `phys1000b_25pct` (arm B, the seed replication) — neither is a result until it carries `_run.json` |
| Open issues | **9 open** (37 + #60 filed, 28 closed). Directive issues, BUILD half done, converged-run measurement open: **#48** (re-mode measured live: 5,533–5,792 legs/iteration at 25%; emergent ride 0.26–0.31% through it 133 — diagnostics), **#49** (motorbike built §9.52; open: pt reporting split, choice-distinct submodes, taxi 4.4), **#50** (inventory measured; open: the modelled table + the mode × age acquisition), **#30** (decomposition measured twice — §9.57 adds the physical-model view: 66% of walk legs are whole-trip at mean ~11 km; open: the generation mechanism). **#60 RESOLVED IN-TREE (§9.58, 21 Aug)**: the suspicion was verified FALSE (the qsim never reads `disallowedNextLinks`; routers apply them per mode) — the 491,349 refusals were first-hop topology breaks from activities on walk-less links + MATSim's silent nearest-link fallback; four repairs built (trunk walkable per the actual road rules, reverse walk/bike complements on one-ways, `ActivityLinkAssigner` pins every activity to a link its person can use, SubtourModeChoice person-only — 405 externals had walked off their cordon counts), probe-verified 0 refusals on both the old and the regenerated demand; closes when the PR lands. **#62 NEW** (deep city-agnosticism: the input contract itself is Newcastle-shaped — layers/output schemas, ABS-shaped readers, fixture-test blind spots). **#63 NEW** (deliverable-0b backlog: ranked derivations incl. two attended acquisitions and ~25 reclassifications). #49's Tier R (pt reporting split) is DONE in-tree. Carried: #14 + #9 (await a completed base arm + C5), #28 + #31 (ride measurement ledgers — #48's build half advanced by §9.60). #54 closed by PR #58 (events-based accounting, conservation closes on all four runs it was tested on). |
...
| # | Task | Issue | State |
|---|---|---|---|
| **4.5.1** | **Physical ride** — every ride trip a passenger physically in a car, share tuned to the observed 20.60% | **#48 — FIRST** | 🟢 **BUILT end-to-end, probe-verified (§9.53 + §9.55 + §9.60)**: `JointRideEngine` boards paired passengers; unpaired ride re-modes to physical walk (emergent share); **the non-household-lift gap now has its mechanism (§9.60, owner-directed)** — M0: a booked passenger physically WAITS at the meeting point (the old missed-gone/absent classes measured 0 at the probe); M1: unbound observed-rate escort tours re-targeted to driverless-household passengers (WEEKDAY 55,280/55,614 bound; sampling co-clusters bound household pairs). Open: the converged-run measurement (where emergent ride settles vs 20.60%), M2 (driver detours) gated on that measurement, the summariser's stuck-attribution defect (fixed by #54) |
...
| 4.5.0 | **RELAUNCHED 21 Aug (third session, §9.62)** as two concurrent arms under the §9.59 pattern: arm A `phys1000a_25pct` (the base arm) + arm B `phys1000b_25pct` (the seed replication, seed 20260811). Watched every ~50 iterations; close-out per the recorded one-pass list when arm A completes. The first attempt (run to iteration 136, stopped at owner instruction) stays quarantined §9.57 | #14 | 🟢 in flight |
