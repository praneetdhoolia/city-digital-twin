...
| Blocking state | **A RUN IS IN PROGRESS - THE MACHINE IS NOT FREE.** `results/20260830T184955_300it_10pct` (S2, WEEKDAY, 10%, 300 it, innovation off at 240, overlay `f20_gate_10pct`, Task Scheduler `citysim_run_20260830T184954`) is the **FIRST F20 ARM (§9.129)**, launched 30 Aug 18:49 on the demand re-bound under the same-bucket rule with the carves solved on the drawn pool; first gate at iteration 100 (~23:50 at ~3 min/it), then 200 and 300, per mode, under the standing /goal directive. The F19 arm `aborted_20260830T170743_300it_10pct` was stopped at iteration 27 (its sample kept named drivers at 12.4% and the motorbike carve at 5.5%). NEVER compare across the F20 boundary, a sample fraction or a network build. |
| Committed data package | **498 files** (the disclosed pt boardings targets joined it, §9.130; the three shared-ride binding tables, §9.124) in [`data/MANIFEST.csv`](../data/MANIFEST.csv) · `check_manifest.py` passes · `check_package.py` **ALL PASSED** (2 standing warnings; it prints its own check count, which is why one is not restated here) — **but it was NOT passing on arrival this session and this cell said it was** (§9.117): two failures stood on `main`, a `decisions_ref` naming a record that was never written (§9.93, cited by 2 fields) and three false `consumers` claims. Both classes are repaired; **run the suite before believing this cell** — now a portable harness over city-owned expectations ([`cities/newcastle/tests/package_expectations.json`], #62 B4) |
...
| 2 | Open data package | 🟡 **498** files, provenance, licence, lineage — the ODbL/CC-BY split is recorded per row but not yet published (task 7.4) |
...
| 4.15.34 | Why heavy rail boards five times the disclosed entries at suburban stations | #49 | **NEXT** — measured at the F20 gate: who boards at Hamilton / Waratah / Adamstown, their OD, trip length and alternatives |
| 4.15.32 | **Run the first F20 arm and gate it per mode at 100 / 200 / 300** | #48 #86 #91 #93 #49 #30 #94 #82 | **RUNNING** — `20260830T184955_300it_10pct`, launched 18:49 detached (`citysim_run_20260830T184954`) after smoke `20260830T184637_2it_1pct`; F19 arm stopped at iteration 27; **iteration 0 verified the sample's composition**: 62,134 persons, 175 motorbike-locked (F19: 52; 169 expected), 23,040 named drivers (F19: 28,244; ~22,800 expected), motorbike departures 581 (F19: 159), pair rate 0.977, 2,864 passengers on 2,683 detours; first gate at iteration 100 (~23:50) |
