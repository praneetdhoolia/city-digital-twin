...
|---|---|---|
| SCATS signal phasing | Corridor run time swings 38% between no priority and full priority (S2 vs S2b) — the largest single uncertainty in the model | Swept, and stays `unobtained` for the 14 modelled sites. The [`design/signalling/`](design/signalling/README.md) dossier maps the mechanics and every data route; operated history for two NON-modelled sites is ARCHIVED (`data/raw/planning_tia/`, the decided free-TIA route, §9.76); the native signal build is BUILT INERT (#73, §9.76) |
...
| 4.7.5 | Level crossings at Adamstown + Islington via `NetworkChangeEvents` | #68 | ✅ **BUILT INERT 25 Aug (§9.76)** — derived from OSM barrier tags, per-link restoration, sites phase-offset; Stewart-Avenue exclusion ASSERTED (3.8/2.3 km clear); activation needs `bin_size` ≤300 s |
| 4.7.6 | Charging dwell natively | #74 | ✅ **BUILT INERT 25 Aug (§9.76)** — concurrent-with-boarding DECIDED; derived `transitSchedule_dwell.xml.gz` per scenario; anchors byte-preserved |
| 4.7.7 | Corridor signals + tram priority natively | #73 | ✅ **BUILT INERT 25 Aug (§9.76)** — 14 systems × 10 scenarios generated; BOTH toy probes PASS on the Maven stack; double-count rule landed both halves; per-green discharge reported (7.1–7.9 veh/green @25%). **OPEN on #73: movement-level lanes (turn-lane coverage 16%), S3 bus-priority detection, activation** |
| 4.7.8 | Taxi/rideshare priced mode | #49 | ✅ **BUILT INERT 25 Aug (§9.76)** — one blended mode on the archived Fares Order 2025 ($5.00 + $2.52/km urban, correcting the dossier); flagfall via `FareChargeHandler`; wait in the constant; band a CONSTRAINT. Activates with the vocabularies at the boundary |
