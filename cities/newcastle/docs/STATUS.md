...
| Blocking state | **NO RUN IS IN PROGRESS. THE MACHINE IS FREE.** The F8 arm `20260826T233658` was STOPPED ON INSTRUCTION at iteration 163 of 1000 (13.4 h, 257.89 s/it median), before its iteration-200 gate, and is closed out as `aborted_20260826T233658_1000it_25pct` with a measured cause. No approval is outstanding and NONE IS STANDING - approvals are spent on use. **The two repairs are now MEASURED and both work**: scored at iteration 150 on the fit basis, mean abs error 10.991 (F6 unfixed) -> 10.460 (§9.81 ride ratchet) -> 10.348 (§9.82 escort coherence), every category improved and none regressed, ride 0.61 -> 1.39 -> 1.61. **Neither is sufficient**: together they close 7.8% of the ride gap, and the escort gain decays with iteration. **The active question is no longer WHICH MODE IS OVER-CHOSEN but WHY THE DEMAND CANNOT SUPPLY THE TARGET** (§9.83): every B2 trip has `party_size = 1`, escort-bound travel is 5.4% of trips against an observed 20.6% vehicle-passenger share, and the measured occupancy constraint (1.3503) agrees with the measured passenger share on a ~14x shortfall. The declared, swept lever is `B.activity.escort_binding_nonhh_scope` (§9.60), currently `same_zone`. Secondary and measured but NOT built: taxi has no availability gate at all, `age` reaches nothing, and gradient reaches mode choice through nothing on a network where 30.5% of edges exceed 4% grade. `E.replication.n_replications` and the warm-restart ruling stay open |
| Committed data package | **492 files** in [`data/MANIFEST.csv`](../data/MANIFEST.csv) · `check_manifest.py` passes · `check_package.py` **ALL PASSED** (2 standing warnings; it prints its own check count, which is why one is not restated here) — now a portable harness over city-owned expectations ([`cities/newcastle/tests/package_expectations.json`], #62 B4) |
| Input registry | **384 fields** (§9.88: +`A.signals.control_regime` and six `A.signals.scats.*` algorithm parameters, all bound into the emitted `scats` module; §9.87: +`CAL.pt_split.window_months`, +`CAL.mode_split.commute_transfer_tolerance`, +`CAL.truck.count_year_from` and the two `CAL.gate.*` acceptance thresholds; §9.85: +`RUN.replanning.time_mutation_range_s` and +`B.ride.bound_pairing_window_min`, derived from it; §9.84: +13 for the joint-tour binder, the taxi/bike age gates and gradient link speed; §9.82: +`B.ride.escort_coherence_rate`; §9.77/§9.78: +`A.crossings.representation` and `A.signals.tsp.priority_group` gates, +`RUN.routing.pt_submode_scoring`, +`B.census.thin_cell_min_journeys`; seven 0b source upgrades incl. CWANZ bike availability 0.493) — every one with units, provenance and a sweep, held-fixed rule or derived identity; ledger **0** with `--strict` gating CI |
