...
**Last updated:** 30 August 2026, SIXTEENTH session (**§9.120**: the per-iteration twelve-mode reader now derives the linked-trip table from any iteration's experienced plans, validated exactly against the trips table; the F14 arm died mid-iteration 38 of a console stop (`0xC000013A`) whose trigger no record on this machine can name; its own iterations measured that residents PLAN ride on 22.5% of trips and realise 9.1% because a third of planned ride legs had no declared driver and the declared pairs decayed under independent time mutation, that 65% of cyclists held no bike-free plan in memory, and that 6.7% of planned trips were never experienced; three repairs that invent nothing - per-trip `boundRideTrips`/`boundDriveTrips` gated in mode choice, declared passengers re-timed to their driver, and the seed as the FULL CHOICE SET (`B.mode.seed_method`, plan memory 8) - open **family F15**, and the first F15 arm is running at 10% x 300 iterations). PRIOR: FIFTEENTH session (**§9.93 reconstructed, §9.116–§9.119**: TWO REPRODUCIBILITY FAILURES ON `main`, BOTH INVISIBLE TO EVERY GATE CI RUNS, AND A CRASH THAT HAD KILLED FIVE ARMS TURNED OUT TO BE MATSIM CREATING A STATE MATSIM REFUSES. **§9.116**: the §9.111 candidate-pool filter had been committed in `b65d280` WITHOUT its rebuild, so the committed builder could not regenerate the committed demand - caught from the committed build report, where `candidates` 201,931 with `candidates_unservable` absent is a report the committed builder cannot write. Both queued fixes applied together and all three day types rebuilt (**family F14**): joint bindings **74,663 → 82,384**, candidates 201,931 → 146,260, `p_thin` 0.8565 → **1.0000**, so joint binding is now **supply-limited by servable candidates** rather than thinned - a different regime, WEEKDAY only. §9.111's estimate of “roughly 110,000” is superseded by the measurement. `B.motorbike.trip_share` 0.0036 → **0.0024064**, `assumed` → `derived`, and `build_mode_targets.py` now ASSERTS its two declared observations against the acquired sources on every build. **§9.117**: `check_package.py` - the local-only gate - was FAILING while the board, the brief and a dated row all called it green; a `decisions_ref` named §9.93 and no such section existed, and three `consumers` claims were textually false. §9.93 is RECONSTRUCTED from evidence already committed, introducing no new number. **§9.119**: the subtour-mix exception that killed FIVE arms is MATSim's own mode choice giving a degenerate one-trip child subtour a non-chain mode and leaving the PARENT holding car+pt - measured at **20 clean→mixed plans in one round against 8 arriving mixed**, after THREE mechanisms were argued from code and all three refuted. Repaired by two refusals that invent nothing. NOTHING IS A RESULT: the first arm to survive it is running now and has never reached its innovation cutoff.)
...
| Blocking state | **A RUN IS IN PROGRESS - THE MACHINE IS NOT FREE.** `results/20260830T163010_300it_10pct` (S2, WEEKDAY, 10%, 300 it, innovation off at 240, overlay `f18_gate_10pct`, Task Scheduler `citysim_run_20260830T163010`) is the **FIRST VALID F18 ARM (§9.126, §9.127)**, launched 30 Aug 16:30 on a 10% sample asserted at 62,134 persons (F17's count exactly); the chain's smoke step RESUMED the 16:10 probe `20260830T161030_2it_1pct` (same controler and values hashes - the harness does not see a changed population as a new run), so the rebuilt plans' only untested difference is the `sharedDriverHousehold` attribute; gate it per mode at 100 / 200 / 300 with `report_mode_ridership.py --trend`, ~106 s/it, first gate ~19:30. `aborted_20260830T161243_300it_10pct` was launched at 16:12 and **stopped at iteration 2 on a defective sample**: the household sampler's union-find over `liftHousehold` made the sampling unit the connected component, which the shared-ride bindings turn into giant lumps - 31,262 persons kept at 10% against F17's 62,134. Repaired at the binder (a passenger is bound only to drivers whose household unit hash is at or below theirs, so any nested sample that keeps the passenger keeps the driver) and at the sampler (shared drivers excluded from the clusters); the rebuild asserts the 10% sample within 8.5-11.5% of persons before the smoke and the launch. PRIOR TEXT: `results/20260830T161243_300it_10pct` (S2, WEEKDAY, 10%, 300 it, innovation off at 240, overlay `f18_gate_10pct`, Task Scheduler `citysim_run_20260830T161242`) was the **FIRST F18 ARM (§9.126)**, launched 30 Aug 16:12 after smoke probe `20260830T161030_2it_1pct`: F17's run side unchanged on the demand §9.122-§9.125 built - shared rides for the car-less quarter, the repaired carves, residents who drive trucks. Gate it per mode at 100 / 200 / 300 with `report_mode_ridership.py --trend`; ~106 s/it, first gate ~19:15. **The F17 arm `20260830T141222` was STOPPED at iteration 60** when the F18 build completed: it had converged **car to 59.32% (+1.7%) and walk to 14.88% (+11%) by iteration 50** - the first honest arm since F4 - and its residues are the causes F18 carries (car-less quarter: ride -51%, bike +275%, bus +87%, heavy rail +70%), the corridor (#30) and the taxi fleet. PRIOR TEXT: `results/20260830T141222_300it_10pct` (S2, WEEKDAY, 10%, 300 it, innovation off at 240, overlay `f17_gate_10pct`, Task Scheduler `citysim_run_20260830T141221`) was the **FIRST F17 ARM (§9.121, #94)**, launched 30 Aug 14:12 after the smoke probe `20260830T140930_2it_1pct` ran the new router (254 direct-walk decisions logged); identical to the F15/F16 arms in every run parameter and differing from F16 in the router's direct-walk basis alone. Gate it per mode at 100 / 200 / 300 with `report_mode_ridership.py --trend`; ~120-150 s/it, so the first gate lands ~17:45. Two injector deaths precede it - `aborted_20260830T140646_2it_1pct` and `aborted_20260830T140727_300it_10pct`, a Guice `CreationException` (the raptor module's provider unbound under JIT-disabled injection), fixed and recorded; `run_failure.py` now reads such a bare throwable. **The F16 arm `20260830T132843` was STOPPED on the gate at iteration 17**: it showed the seed-order artefact gone (car the best-scored plan for 61.1% of car-available residents) and then that **38.3% of residents' pt-plan trips were walk-only** - SwissRailRaptor's BEELINE direct walk, which crosses the harbour the network does not: of 256 harbour-crossing trips in residents' pt plans (by bank - a first radius reading had the wharves swapped, corrected in §9.121) 174 were routed as a walk and 23 with a ferry leg (#94), and 45.5% of all such walks are over 3 km on the network. On the F17 arm at the same depth 209 of 359 crossings route with a ferry leg and realised ferry reads 30 trips against 3. Repaired at the root: the direct walk is now routed on the walk network and compared with the raptor's own rule (`citysim.NetworkDirectWalkPtRouter`, `RUN.transit_router.direct_walk_basis` = `network`). PRIOR TEXT: `results/20260830T132843_300it_10pct` (S2, WEEKDAY, 10%, 300 it, innovation off at 240, overlay `f16_gate_10pct`, Task Scheduler `citysim_run_20260830T132842`) was the **FIRST F16 ARM (§9.121)**, launched 30 Aug 13:28; identical to the F15 arm in every run parameter, differing in the seed order alone (selected seed plans: car 34.6%, walk 19.1%, pt 18.3%, taxi 14.6%, bike 7.1%, ride 2.4%). **The F15 arm `20260830T124711` was STOPPED on the gate at iteration 13** with its cause measured: the seed executed the car plan first for every car-available resident, iteration 0 gridlocked (74.7% car, 6,820 cars stuck at 30:00), and every car plan in memory kept that score while walk/bike/pt were scored on quarter-traffic roads - the bike plan beat the car plan by +68 utils on average. What the arm DID establish: the §9.120 ride repairs work - 13,580 of 15,295 ride legs paired (88.8%, 13,029 on identity) against 41% on F14. The repair draws each person's first-executed seed plan uniformly (family F16). PRIOR TEXT: `results/20260830T124711_300it_10pct` (S2, WEEKDAY, **10%**, **300 it**, seed 20260810, 30g, innovation off at 240, overlay `f15_gate_10pct`) is the **FIRST FAMILY-F15 ARM (§9.120)**, launched 30 Aug 12:47 detached under Task Scheduler `citysim_run_20260830T124710`; **measured 151.7 s/it median at iteration 2** (not the F13 10% arm's ~100 s/it - the choice-set seed loads pt, taxi and walk early), so the iteration-100 gate lands ~17:15 and the run ~13 h. Gate it per mode at 100 / 200 / 300 with `python src/analyse/report_mode_ridership.py --run results/20260830T124711_300it_10pct --it <n>` - **the reader now derives the twelve-mode table from any iteration's experienced plans** (every 10 iterations), validated exactly against the trips table (§9.120). F15 carries: `ride` offered only on trips with a declared driver (`boundRideTrips`) and declared drivers held on car (`boundDriveTrips`), both gated in `GatedSubtourModeChoice`; a declared passenger re-timed to the driver's departure in `RidePairingEngine`; the seed as the FULL CHOICE SET (`B.mode.seed_method`, one plan per usable mode, plan memory 8); the motorbike carve solved on eligible persons' own trip counts. What the arm must answer: does ride realise what the demand binds (the ~11%-of-trips ceiling, #86), does car's planned share stop being masked by fallback drives, do walk and bike fall once every mode is scored, and does the model settle inside 250 iterations. **The F14 arm `20260830T083019` is DEAD**: it vanished mid-iteration 38 at 12:03 with `0xC000013A STATUS_CONTROL_C_EXIT` recorded by Task Scheduler, no exception, trigger NOT established (scheduler log disabled - enabling it needs elevation; no power event; coincident with the sixteenth session's start) - closed out as `aborted_20260830T083019_1000it_25pct` with the measurements it yielded at iterations 1/10/20/30 (§9.120). NO approval stands beyond this arm; approvals are spent on use. PRIOR TEXT: `results/20260830T083019_1000it_25pct` (S2, WEEKDAY, 25%, 1000 it, seed 20260810, 40g, cutoff 800) was the **first arm to survive the §9.119 subtour-mix defect**: five arms died at iteration 3 on `IllegalStateException: Subtour contains a mix of chain- and non-chainbased modes`, and this one **cleared iteration 6** with 5 proposals refused and 5 plans stood aside, at a median **288 s/it** (~80 h for 1000). Three mechanisms were argued from the code and all three refuted before a two-sided diagnostic measured the real one - **MATSim’s own mode choice creates the state it later refuses** (20 clean→mixed plans in one round against 8 arriving mixed). The demand holds 99 mixed subtours of 1,138,887 and **0 of the fatal single-excursion shape** (#96). EARLIER F14 ARMS, all dead and all recorded with their causes: `aborted_20260830T022430`, `aborted_20260830T024952` (both the subtour mix), `aborted_20260830T082015` (diagnostic, stopped). PRIOR TEXT: `results/20260830T022430_1000it_25pct` was the **FIRST F14 ARM** (S2, WEEKDAY, 25%, 1000 iterations, seed 20260810, 40g, innovation cutoff 800), launched 30 Aug 02:24 and **verified past `PersonPrepareForSim` into iterations** per #70. Iteration 0 ran in 240 s, consistent with the measured 25% pace, so 1000 iterations is ~66 h. It carries the §9.116 rebuild (#92 + #93). Gates every 100 iterations per the standing directive; **read the TREND, not the level** (§9.108). The previous arm `aborted_20260829T172145_1000it_10pct` was 10% and **nothing in it is comparable with this one** (§9.10, §9.12). PRIOR STATE, for the record: **NO RUN IS IN PROGRESS. THE MACHINE IS FREE.** Arm `20260829T172145_1000it_10pct` (10%, 1000 it) was **stopped on the /goal iteration-100 gate** at iteration 101 after 3.2 h at 100.4 s/it and is closed out as `aborted_20260829T172145_1000it_10pct` with its cause, its twelve-mode reading and its planned-share trend recorded. **No approval is outstanding and NONE IS STANDING** - approvals are spent on use. **BOTH QUEUED FIXES ARE NOW APPLIED AND THE DEMAND IS REBUILT (§9.116, family F14)**: the joint-binding candidate-pool filter (#92, §9.111) and the motorbike carve conversion (#93, §9.115). The rebuild was **not optional** - `b65d280` had already committed the filter without it, so the committed builder could not reproduce the committed demand and no gate could see it. Measured on the rebuild, WEEKDAY: joint bindings **74,663 → 82,384**, candidates 201,931 → 146,260, `p_thin` 0.8565 → **1.0000**, so joint binding is now **supply-limited by servable candidates** rather than by thinning - a different regime, and a WEEKDAY property only (SAT 0.6216, SUN 0.5955). §9.111’s “roughly 110,000” estimate is superseded by the measurement. **The active question is no longer which mode is over-chosen**: car, walk and pt are converging (§9.108) and need iterations, not repairs; ride and bike are one upstream demand defect (§9.109/§9.111/§9.114); light rail has a 1.06% corridor market (§9.103) and ferry a 450-trip detour market it does not capture (#94, §9.112), with supply ruled out for both on departures (§9.113). `E.replication.n_replications` and the warm-restart ruling stay open |
| Committed data package | **497 files** (the three shared-ride binding tables joined it, §9.124) in [`data/MANIFEST.csv`](../data/MANIFEST.csv) · `check_manifest.py` passes · `check_package.py` **ALL PASSED** (2 standing warnings; it prints its own check count, which is why one is not restated here) — **but it was NOT passing on arrival this session and this cell said it was** (§9.117): two failures stood on `main`, a `decisions_ref` naming a record that was never written (§9.93, cited by 2 fields) and three false `consumers` claims. Both classes are repaired; **run the suite before believing this cell** — now a portable harness over city-owned expectations ([`cities/newcastle/tests/package_expectations.json`], #62 B4) |
| Input registry | **411 fields** (§9.125: +`CAL.mode_split.truck_driver_journey_share` 0.0050729 (measured, the target LGA's G62 Truck cell, asserted on every build) and +`B.truck.resident_trip_share` 0.002993 (derived by the motorbike carve's identity) - residents who drive a truck for a living, carved and locked like motorcyclists; §9.124: +`B.ride.shared_lift_scope` = `same_sa2_od` - the fourth binder pass, car-less residents' direct tours bound to non-household drivers on the same SA1-to-SA1 trip, built and queued; §9.122: +`B.motorbike.carve_resolution` = `sa1_thinned` - the carve at the resolution G62 observes it, built and queued for the next family; §9.121: +`RUN.transit_router.direct_walk_basis` = `network` (derived - the walk the router compares must be the walk the agent would make) and +`RUN.transit_router.direct_walk_factor` 1.0, MATSim's default that had reached every config undeclared; §9.120: +`RUN.transit_router.search_radius_m` 1000 and +`RUN.transit_router.extension_radius_m` 200 - MATSim's jar defaults that had governed every submode's access reach, the ferry's included, while declared nowhere; +`B.mode.seed_method` = `full_choice_set`, `RUN.replanning.max_agent_plan_memory` 5 → 8 inside its sweep, `B.taxi.min_unaccompanied_age` now also read by the plans builder; §9.115/§9.116: +`CAL.mode_split.vehicle_driver_level`, +`CAL.mode_split.motorbike_driver_journey_share` (both measured, and now ASSERTED against their acquired sources on every `build_mode_targets.py` run), with `B.motorbike.trip_share` moved `assumed` → **`derived`** from the two of them; §9.106: +`B.mode.walk_feasible_km`, +`B.mode.bike_feasible_km` (both derived); §9.105: +`B.ride.unpaired_fallback`; §9.100: +`CAL.pt_split.station_scope`, +`CAL.pt_split.break_ratio`, +`CAL.pt_split.lr_observed_stop_share`; §9.99: +`A.taxi.fleet_representation`, +`B.taxi.vehicle_trips_per_day`, +`B.taxi.fleet_size` (derived), +`B.taxi.max_wait_min`, +`B.taxi.deadhead_min`; §9.91: +`CAL.taxi.lga_concentration`; §9.90: +`A.crossings.closure_source`, +`A.crossings.freight_closures_per_day`, +`A.crossings.rail_match_radius_m`; §9.88: +`A.signals.control_regime` and six `A.signals.scats.*` algorithm parameters, all bound into the emitted `scats` module; §9.87: +`CAL.pt_split.window_months`, +`CAL.mode_split.commute_transfer_tolerance`, +`CAL.truck.count_year_from` and the two `CAL.gate.*` acceptance thresholds; §9.85: +`RUN.replanning.time_mutation_range_s` and +`B.ride.bound_pairing_window_min`, derived from it; §9.84: +13 for the joint-tour binder, the taxi/bike age gates and gradient link speed; §9.82: +`B.ride.escort_coherence_rate`; §9.77/§9.78: +`A.crossings.representation` and `A.signals.tsp.priority_group` gates, +`RUN.routing.pt_submode_scoring`, +`B.census.thin_cell_min_journeys`; seven 0b source upgrades incl. CWANZ bike availability 0.493) — every one with units, provenance and a sweep, held-fixed rule or derived identity; ledger **0** with `--strict` gating CI |
...
| 2 | Open data package | 🟡 **497** files, provenance, licence, lineage — the ODbL/CC-BY split is recorded per row but not yet published (task 7.4) |
...
| 4.15.27 | **Run the first valid F18 arm and gate it per mode at 100 / 200 / 300** | #48 #86 #91 #49 #30 #93 #94 #82 | **RUNNING** — `20260830T163010_300it_10pct`, launched 16:30 on 62,134 sampled persons (asserted); shared pass re-bound under the hash rule (WEEKDAY 98,549 servable, 59,718 bound, 0 short); first gate ~19:30 |
