...
| 4.6.9 | **Next base arm on the repaired demand** — 25% × 1000 × WEEKDAY; re-measure every mode against its observed value. Two launch attempts died silently 24 Aug (§9.72, #70): launch from an interactive shell outside the agent session, verify past `PersonPrepareForSim`; **arm B only if arm A solo paces at 217–253 s/it (iterations 2–5)** | #48 #30 | ⬜ **needs fresh stated-cost approval (~65–67 h/arm); launch-blocked by #70's constraint, not by the model** |
