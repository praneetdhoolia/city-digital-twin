...
| Open issues | **9 open** (44 filed — numbers shared with PRs — 35 closed): **#70** (NEW — session-spawned runs die silently; detached launch path, §9.72) · **#48** (ride — the §9.68 repair is BUILT and probe-verified; open until the converged arm measures where emergent ride settles vs 20.6) · **#30** (walk — the §9.69 mixture is BUILT, band shares exact in the build; open until the converged arm re-measures walk vs 13.4) · **#49** (Tier R done; Tier C + taxi 4.4 — the 2025 evidence base is complete on the issue, build owner-sequenced) · **#50** (modelled table DELIVERED from arm A — children 46–50% bike / 0% ride; mode × age acquisition open) · **#62** (city-agnostic input contract) · **#63** (0b backlog — pre-LR lanes and the EPV check measured §9.71; remaining: route-110 speed, parking capacities, JTW extract, ~25 reclassifications) · **#66** (machine stall, monitoring) · **#68** (NEW — Adamstown/Islington level-crossing closures, designed §9.70). #65 closed 24 Aug when PR #67 merged. |
