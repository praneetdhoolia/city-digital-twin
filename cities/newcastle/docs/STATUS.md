...
**Last updated:** 29 August 2026, FOURTEENTH session (**§9.100–§9.102**: THE GATE FIRED AT ITERATION 100 AND EVERY DEFECT IT FOUND WAS IN THE YARDSTICK AGAIN. Arm `20260829T172145_1000it_10pct` - the first carrying the §9.99 taxi fleet - was stopped at its iteration-100 gate with 9 of 11 fittable modes past 20%. **The fleet works**: taxi reads 1.5394% against 9.3525% at the same depth on the previous arm, with the engine refusing 32–36% of requests against a sample-scaled fleet of 80. Three yardstick defects then accounted for most of what the gate flagged. **§9.100: the PT composition counted a light rail stop belonging to ANOTHER CITY** (30,241 boardings, 15.6% of the light rail leg, while five of this line’s six real stops contributed nothing), **pooled THREE LGAs of train boardings** (only 53.7% in the target LGA the HTS level describes), and **took its window from inside a broken bus series** that falls 319,770 → 37,414 in one month while every other contract region continues normally. Station membership is now DERIVED from the city’s own schedule and boundary, the window must be contiguous and break-free, and a line reported at one stop is scaled by a measured share. **Heavy rail’s −1.5% “fit” was an artefact of the contaminated split; corrected, NO mode is inside the bar.** **§9.101: truck was scored against a target its own basis calls not comparable** - a network-wide share against a freight-route observation - and on the ground the target was measured it reads **+5.4%**, not −49.6%, though 20 of the 24 classifying stations are HOLDOUT and were not opened. **§9.102: ride’s pairing rule cannot represent a drop-off en route**, and `route_contains` was BUILT to - then measured against a committed `both_links` control differing in that field alone, and it does NOT help: pair rate 0.5069 -> 0.5113 while ride mode share went the WRONG way, 9.6748 -> 9.4513. The driver's PLAN carries no escort stop, so the driver's PATH does not pass the passenger either. **9.92's ruling stands and `both_links` remains the value**; the new rule is a declared sweep member. That is the THIRD pairing-side lever measured real-but-marginal - stop looking at the pairing engine. **9.104: resume matched two runs differing in a declared value and handed back the wrong one**, which would have reported this pair as identical; the run record now carries `values_sha256` over every resolved registry value. The decomposition that matters: measured on ONE basis both sides, bus is +18.5% and heavy rail +20.3% of PT boardings while **light rail is −95.9% - 148 boardings of 21,429 against 550 light rail trips a day of supply**. The PT LEVEL is +145% and the whole within-PT error is the intervention itself. NOTHING IS A RESULT: no arm in F11, F12 or F13 carries a `_run.json`.)
...
| Blocking state | **NO CONVERGENCE ARM IS IN PROGRESS.** Arm `20260829T172145_1000it_10pct` (10%, 1000 it, the first with the §9.99 taxi fleet and the §9.95 window) was **stopped on the /goal iteration-100 gate** at iteration 101 after 3.2 h at 100.4 s/it, and is closed out as `aborted_20260829T172145_1000it_10pct` with its cause, its twelve-mode reading and its planned-share trend recorded. **No approval is outstanding and NONE IS STANDING** - approvals are spent on use. What the gate established: the taxi fleet BINDS (taxi 9.3525% → 1.5394% depth-matched, target 0.9916%), and the PLANNED shares converge for car 0.3409 → 0.4371 and walk 0.2888 → 0.2161 across 95 iterations **while ride falls 0.1903 → 0.1474, away from its 0.2060 target** - the one clearly diverging mode and the reason §9.102 exists. **The active question is no longer which mode is over-chosen but why PUBLIC TRANSPORT carries +145% of its observed level while the LIGHT RAIL - the intervention this study exists to test - carries 148 boardings of 21,429 (−95.9% of its observed share) against 550 scheduled trips a day.** Supply is not the explanation and the yardstick is no longer the explanation; the composition is now right for bus and heavy rail to within a quarter. **9.107 CORRECTS 9.103 and 9.106 on where the walk defect lives**: the HTS's own DISTANCE_BY_MODE column - an observed anchor never used - puts resident trips at 1.11x, total km at 1.20x and mean trip length at 1.08x, and 16.31% of modelled trips are under 1 km against an observed walk-only 13.4%, so **the demand HAS the short trips and destination placement cannot account for walk's geometry**. The defect is ALLOCATION: only 39.5% of sub-1 km trips are walked while car takes 39.2%, and walk's resident mean is 6.66 km against an observed 0.70. Modes are assigned per SUBTOUR, so a car chain drives its 800 m leg and a walk chain walks its 20 km leg. **The next lane is scoring a per-mode MEAN TRIP LENGTH, which this package observes and has never fitted against.** **9.108 REFRAMES the whole gate**: read as a TREND rather than a level, **car (0.3409 -> 0.4429), walk (0.2888 -> 0.2116) and pt (0.0688 -> 0.0603) are ALL converging** and at their measured rates arrive between iterations 200 and 350; walk's GEOMETRY converges with them (mean 8.12 -> 6.66 km, trips 21,475 -> 15,955). **The genuinely diverging modes are ride, bike (flat at ~10.0 km against an observed 5.2 on BOTH share and geometry) and motorbike**, plus light rail and ferry on their own structural grounds. **NO ARM IN F11, F12 OR F13 HAS EVER REACHED ITS INNOVATION CUTOFF**, so no post-cutoff level for twelve modes has ever been read. **THE NEXT ACTION IS ONE ARM RUN TO ITS CUTOFF, which needs a fresh stated-cost approval (~28 h at 10%) and NONE STANDS.** `E.replication.n_replications` and the warm-restart ruling stay open |
