...
| **P5** Scenario runs | ⬜ not started | — | **SUMO descoped 25 Aug (§9.74; execution #72)** — the corridor questions land natively in MATSim (#73/#74/#68); the phase's substance is the S0–S6 scenario runs themselves. |
...
|---|---|---|
| SCATS signal phasing | Corridor run time swings 38% between no priority and full priority (S2 vs S2b) — the largest single uncertainty in the model | Swept, and stays `unobtained` for the 14 modelled sites. The [`design/signalling/`](design/signalling/README.md) dossier maps the mechanics and every data route; operated history for two NON-modelled sites found public (TIA route, #78); the native signal build is #73 (§9.75) |
...
| 4.6.9 | **Next base arm on the repaired demand** — 25% × 1000 × WEEKDAY; re-measure every mode against its observed value. Two launch attempts died silently 24 Aug (§9.72, #70): launch from an owner-owned shell, verify past `PersonPrepareForSim`; **arm B only if arm A solo paces at 217–253 s/it (iterations 2–5)** | #48 #30 | ⬜ **needs fresh stated-cost approval (~65–67 h/arm); launch-blocked by #70's constraint, not by the model** |
...
| 4.6.11 | **0b: VoT set checked against TfNSW EPV Jan 2025** — HW/WB/trip-weighted/distance-rate supported within ±30%; HE 9.3 and concession 0.75 divergent (flagged for the owner, values unchanged); EPV bus–LR transfer 3.8 equiv-min noted for the PT-composition lane | #63 | ✅ **DONE 24 Aug (§9.71)** |

### Batch 4.7 — the owner's 25 Aug all-modes-first batch (next session; §9.75)
...
the 4.6.9 arm is the owner's open call (board header).

| # | Task | Issue | Notes |
|---|---|---|---|
| 4.7.1 | Warm restart from written plans (`--warm-start`, provenance-linked; validity caveat recorded) | #75 | harness-only; build BEFORE any 65 h arm |
| 4.7.2 | Machine-readable `_progress.json` per-iteration digest (every mode individually; pace vs the 217–253 s/it band) | #76 | observer-only |
| 4.7.3 | Cross-run index with family and validity labels | #77 | analysis-only |
| 4.7.4 | SUMO descope execution: registry, toolchain, checks, manifest | #72 | a §14 toolchain change when it lands |
| 4.7.5 | Level crossings at Adamstown + Islington via `NetworkChangeEvents` (freespeed floor, `travelTimeBinSize` ≤300 s, PassingQ toy probe; Stewart Ave never double-treated) | #68 | model change — batch boundary |
| 4.7.6 | Charging dwell natively (mapped-schedule offsets + `awaitDepartureTime`; concurrent-vs-additive recorded) | #74 | model change — batch boundary |
| 4.7.7 | Corridor signals + tram priority + lanes natively (signals contrib; Maven run stack = logged toolchain change; double-count rule; per-green discharge check; QSim toy probe) | #73 | model change — batch boundary |
| 4.7.8 | Taxi/rideshare priced mode (measured urban fares; swept rates and wait; band as constraint never target) | #49 | model change — batch boundary; resequenced by §9.75 |
| 4.7.9 | Rung-1 registry sharpening: sweep-basis citations (dossier + TIA evidence), `E.s2b.n_segments` derived from the mapped feed | #63 | docs/registry only |
| 4.7.10 | Frontage volume analyser from physical walk link events (hypothesis B1's fallback instrument) | — | analysis-only; informs the pending 6.1 REWORK |
