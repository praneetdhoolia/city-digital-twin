...
(the first five added 16 September 2026, §9.176, after one handoff placed them by hand; the last three the same day, §9.177, from the twelfth report's process audit):
...
| a harness or emitter change verified against a finished run's own config: IDENTICAL, or the diff naming the parameter | `python src/run/reemit_config.py --run <name>` (the reference city's newest smoke first, then the second city's) |
