...
| The issue ledger: 37 open - 13 `awaiting-run` with a measurement, 24 with an `AWAITING-DECISION:` line, 0 blocking; #197 #198 #199 #200 are fixed on this branch and CLOSE WHEN THE PR MERGES; #196 awaits a decision; the sixteen risks the tenth report named are #202-#217. | `python src/run/issue_gate.py` · `gh issue list --state open` |
