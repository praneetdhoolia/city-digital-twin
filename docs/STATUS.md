...
**Last updated:** 21 September 2026 (fifty-seventh session). Mumbai runs through the harness; its extent is the notified MMR and its 27.06 M-person population is synthesised (§9.204, D13).
The scoreboard is Newcastle's; Mumbai's executable case is still the explicit 1,000-person population with no targets, now launched, recorded and read by the same harness.
...
| P4 calibration | 🟡 | the newest run on disk is `20260921T182708_2it_100pct`, which **RAN TO ITS LAST ITERATION**: the Mumbai structural case through the framework's own harness, two iterations, no claim (§9.204). Newcastle remains at `20260916T063903_250it_25pct`, completed inside F35 (§9.177); three F35 results agree within 0.35 pp. **The next 25 % arm is the first with standing room scaled (§9.203, #237) and opens a new family.** No run approval stands. |
...
| `20260921T165718_2it_100pct-mumbai-smoke` | mumbai | completed | - | 2 | ran_to_last_iteration `_run.json` |
| `20260919T155059_8it_100pct-mumbai-smoke` | mumbai | completed | - | 8 | ran_to_last_iteration `_run.json` |
| `20260919T154848_8it_100pct-mumbai-smoke` | mumbai | failed | - | - | hiredFleet.vehiclesByMode is supplied with role 'vehicles_in_explicit_simulated_population', which C:\Users\Praneet Dhoolia\work\city-dig... |
| `20260919T152454_8it_100pct-mumbai-smoke` | mumbai | completed | - | 8 | ran_to_last_iteration `_run.json` |

221 run directories on disk; `results/INDEX.md` labels every one. A dead run states its cause in its own `_meta.json`.
<!-- generated:runs end -->
...
| **The second city** (§9.201–§9.204): Mumbai runs through the harness (`RUN.scoring.translation = bound_fields`); its extent is the notified MMR (D13); 27.06 M persons synthesised for 2026, no plans from them yet; the CTS/CMP mode splits transcribed; the executable case is still the explicit 1,000-person population, no targets | #239 | [network-and-inputs](positions/network-and-inputs.md), [`cities/mumbai/docs/README.md`](../cities/mumbai/docs/README.md) | plans from the synthesised population through the harness; the first `mode_targets_by_mode.csv` |
