...
**Last updated:** 21 September 2026 (fifty-seventh session). Mumbai runs citywide plans through the harness against derived targets; its fraction is D14 (§9.204, §9.205).
...
| P4 calibration | 🟡 | the newest run on disk is `20260921T220701_2it_0.1pct`, which **RAN TO ITS LAST ITERATION**: the first citywide Mumbai case through the harness, 0.1 % of the core, a structural check whose trip times are the flow identity at 0.001 (§9.205). Newcastle remains at `20260916T063903_250it_25pct`, completed inside F35 (§9.177); three F35 results agree within 0.35 pp. **The next 25 % arm is the first with standing room scaled (§9.203, #237) and opens a new family.** No run approval stands. |
...
| **The second city** (§9.201–§9.205): Mumbai runs citywide plans (1.35 M persons at a 0.05 build fraction) through the harness against thirteen derived targets; the 0.1 % case is a check, not a reading - 140 KB an agent (1 % needs 50 GiB of 63) and 1.8 veh/h a lane at 0.001; D14 decides the fraction | #239 #242 | [network-and-inputs](positions/network-and-inputs.md), [`cities/mumbai/docs/scaling.md`](../cities/mumbai/docs/scaling.md) | a 1 % case's per-agent memory with a leaner agent (D14) |
