...
**Last updated:** 22 September 2026 (fifty-ninth session). The Maritime Board's seven commuter crossings are in Mumbai's feed and ran in a 0.1 % check; the pass-through merge was measured (median link 62.7 → 69.0 m) and, by the user's decision, not applied (§9.207).
The scoreboard is Newcastle's; Mumbai reads at no fraction this host carries. D15 is taken (a 384-512 GB host for a 10 % core), D16 (the OGD key) and D17 (a browser capture from a network the refusing hosts admit) too.
...
| P4 calibration | 🟡 | the newest run on disk is `20260922T031226_2it_0.1pct`, which **RAN TO ITS LAST ITERATION**: the 0.1 % Mumbai check of the feed with the seven commuter crossings (§9.207); the 1 % cases of §9.206 remain the flow-identity measurement, the merge is measured and not applied, D15 takes a 384-512 GB host. Newcastle remains at `20260916T063903_250it_25pct` inside F35 (§9.177). **The next 25 % arm opens a family (#237).** No run approval stands. |
...
226 run directories on disk; `results/INDEX.md` labels every one. A dead run states its cause in its own `_meta.json`.
<!-- generated:runs end -->
...
| **The second city** (§9.201–§9.207): the feed holds every mapped operator and the seven commuter crossings at evidenced capacities; both 1 % cases gridlock on the flow identity and the merge cannot move it (median 62.7 → 69.0 m), so the reading needs D15's host; 40 sources refuse this address (D17), two wait on the OGD key (D16) | #239 | [network-and-inputs](positions/network-and-inputs.md), [`cities/mumbai/docs/scaling.md`](../cities/mumbai/docs/scaling.md) | the 10 % probe on the D15 host, priced by `arm_cost.py` |
