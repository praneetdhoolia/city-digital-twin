...
**Last updated:** 16 September 2026 (fifty-fourth session). Newest reading: the routers pair `20260915T000704_250it_25pct`, the third RESULT (250 of 250, 27.39 h) - **2 of 12 inside 10 %**, six past the stop bar, nothing moved against arm 0 outside one build's noise (§9.176); report #12 lodged on it (`docs/reports/20260916T062246_project_report.html`).
The scoring pair `20260916T063903_250it_25pct` is RUNNING (D7, 33.0 h ceiling); D6 taken - a launch detaches by default; a run now carries its own residents map (#213).
...
| P4 calibration | 🟡 | the newest run on disk is `20260916T063903_250it_25pct`, which is **RUNNING** - the scoring pair (`f35_scoring_msa_25pct`: `RUN.replanning.score_msa_representation` = `at_innovation_cutoff`, 250 it, 25 %, 33.0 h ceiling), launched detached 16 September on the committed build's probe price (383 s an iteration, 27.2 h quoted, D7); read against arm 0 `20260912T202242_300it_25pct` inside F35, like the routers pair `20260915T000704_250it_25pct` (a RESULT, moved nothing, §9.176). The 16 September approval (one probe, one arm up to 35 h) is spent on it |
...
|---|---|---|---:|---|
| `20260916T063903_250it_25pct` | running | F35-the-engines-route-what-they-remode | live | - |
...
1. **The roots rebuild, re-scoped by report #12: hold escort members and joint companions to ride on their bound tours (#86, D12), consume the household-size top-band mean (#196), give bike a distance cost derived from its observed mean trip length (#107, D9) and re-derive the ferry target from the disclosed TPA tap-on series (#94, D8) - after the scoring pair's reading** **(recommended)** - a demand rebuild (~2 h of builds) that opens a family and re-baselines every pair; then a new arm 0 at the same price as above; opens a family; blocked on: the scoring pair 20260916T063903_250it_25pct reaching its horizon (its reading decides MSA on or off for the new arm 0), then a 25 % probe on the rebuilt inputs for the price and a stated-cost approval (9.177: 20.62 % of core legs bound, 59.0 % of bound trips ride on the pair, the seed at 14.9 %; the 17.70 % short-trip supply at the seed; D8-D12 taken 16 September 2026; #30 #86 #94 #107 #145 #196)
...
| **Report #11's open items** (20260914T152907; §9.170, §9.171): #196 folds into the roots rebuild (a decision, D1); the main ruleset requires the nine test jobs (D3, §9.172); the position pages are capped and trimmed, the rule files split, the report ordinals recorded (#203 #204 #205 #207 #208 closed) | #196 #209-#217 | [monitoring-and-gates](positions/monitoring-and-gates.md) | none - decisions and no-run fixes |
