...
**Last updated:** 22 September 2026 (sixtieth session). Mumbai's suburban trains run the printed timetables, its households carry 2026 vehicles from the registration stock, work follows the built volume, every link carries its grade, and every acquisition has a stated use (§9.209).
The scoreboard is Newcastle's; Mumbai reads at no fraction this host carries (D15 host pending). The OGD key is in `.env` and 20 refusing sources landed through the user's VPN; the OGD and TUS series wait on the user's browser.
...
| P4 calibration | 🟡 | the newest run on disk is `20260922T162633_2it_0.1pct`, which **RAN TO ITS LAST ITERATION**: the 0.1 % Mumbai structural check of the timetable feed, the projected vehicles, the weighted destinations and the graded network (§9.209); the 1 % cases of §9.206 remain the flow-identity measurement, D15 takes a 384-512 GB host. Newcastle remains at `20260916T063903_250it_25pct` inside F35 (§9.177). **The next 25 % arm opens a family (#237).** No run approval stands. |
...
229 run directories on disk; `results/INDEX.md` labels every one. A dead run states its cause in its own `_meta.json`.
<!-- generated:runs end -->
...
| **The second city** (§9.201–§9.209): printed timetables as the suburban schedule, published metro windows and headways, vehicles projected from the registration stock, destinations by built volume, grades on every link, a data-use disposition on every catalogue entry; both 1 % cases gridlock on the flow identity, so the reading needs D15's host; the OGD and TUS series wait on the user's browser | #239 | [network-and-inputs](positions/network-and-inputs.md), [`cities/mumbai/docs/README.md`](../cities/mumbai/docs/README.md) | the 10 % probe on the D15 host, priced by `arm_cost.py` |
