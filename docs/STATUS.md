...
**Last updated:** 15 September 2026 (fifty-second session). Newest reading: arm 0 of F35, `20260912T202242_300it_25pct`, the second RESULT (300 of 300, 30.35 h) - **2 of 12 inside 10 %**, six past the stop bar, ride's target above its coverage (§9.169).
The routers pair is RUNNING as `20260915T000704_250it_25pct` on a spent 30.0 h approval, read against arm 0 inside F35 (D4, §9.175); congestion is now measured the way a map app states it and the viewer's reported defects are fixed and measured (§9.175).
...
Read from `20260915T000704_250it_25pct` at **iteration 25** (family `F35-the-engines-route-what-they-remode`, status `running`, 25% sample, launched 2026-09-15T00:07:08, experienced plans (derived; validated against the trips table)). **Not a result** - only a run whose `_run.json` says `ran_to_last_iteration` is one, and this reading is citable at its `reached_iteration` and nowhere past it.
Reproduce: `python src/analyse/report_mode_ridership.py --run 20260915T000704_250it_25pct --it 25` (`--trend` for the direction).

| # | mode | modelled | target | deviation | gate | basis |
|---|---|---:|---:|---:|---|---|
| 1 | car | 50.9347 | 58.3222 | -12.7% | over 10% | share of resident linked trips |
| 2 | ride | 13.3767 | 20.6000 | -35.1% | **STOP** >=20% | share of resident linked trips |
| 3 | walk | 23.0128 | 13.4000 | +71.7% | **STOP** >=20% | share of resident linked trips |
| 4 | taxi | 1.8732 | 0.9916 | +88.9% | **STOP** >=20% | share of resident linked trips |
| 5 | bike | 6.1848 | 2.2084 | +180.1% | **STOP** >=20% | share of resident linked trips |
| 6 | motorbike | 0.3664 | 0.3785 | -3.2% | ok | share of resident linked trips |
| 7 | bus | 3.0727 | 2.3819 | +29.0% | **STOP** >=20% | share of resident linked trips |
| 8 | heavy_rail | 14,376 | 6,529 | +120.2% | **STOP** >=20% | boardings per weekday, all travellers, x1/fraction |
| 9 | light_rail | 1,312 | 2,954 | -55.6% | **STOP** >=20% | boardings per weekday, all travellers, x1/fraction |
| 10 | ferry | 0.0317 | 0.1429 | -77.8% | **STOP** >=20% | share of resident linked trips |
| 11 | truck | 7.2650 | 15.4698 | - | level only | network-wide road-vehicle share (not the target basis; --truck-stations scores it) |
...
| P4 calibration | 🟡 | the newest run on disk is `20260915T000704_250it_25pct`, which is **RUNNING** - the routers pair (`f35_routers_mode_constant_25pct`, `C.raptor.mode_cost_representation` = `mode_constant`, 250 it, 25 %) on a spent 30.0 h approval, iterations 30–33 at 423–467 s against the 422 s price, the ceiling projected near iteration 215 (§9.174, §9.175). The newest reading is arm 0 of F35 (`20260912T202242_300it_25pct`, 300 it, 25 %, no control on), the second result and the control half of the five pairs (§9.169, #172) |
...
| **Five one-field controls; the routers pair is running** (§9.164, §9.169, D1 §9.172, §9.174): scoring, choice set, routers, service quality (#175), pt submodes (#49). Arm 0 is the control half of all five; `20260915T000704_250it_25pct` is read against it at its end inside F35 (D4, §9.175) | #172 #174 #49 | [seed-and-choice-set](positions/seed-and-choice-set.md) | the routers pair's reading at its horizon or its ceiling, coverage on both sides |
