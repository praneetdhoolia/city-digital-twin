...
**Last updated:** 22 September 2026 (fifty-ninth session). Mumbai's feed carries the seven commuter crossings and Metro 2A/7 in their published windows; the merge was measured and not applied; 22 refusing sources landed as dated Internet Archive copies (§9.207, §9.208).
The scoreboard is Newcastle's; Mumbai reads at no fraction this host carries. D15 is taken (a 384-512 GB host for a 10 % core), D16 (the OGD key) and D17 (a browser capture from a network the 17 uncaptured hosts admit) too.
...
227 run directories on disk; `results/INDEX.md` labels every one. A dead run states its cause in its own `_meta.json`.
<!-- generated:runs end -->
...
| **The second city** (§9.201–§9.208): the feed holds every mapped operator, the seven commuter crossings and Metro 2A/7's published windows; both 1 % cases gridlock on the flow identity and the merge cannot move it (median 62.7 → 69.0 m), so the reading needs D15's host; 17 sources have neither a reachable host nor an archive capture (D17), two wait on the OGD key (D16) | #239 | [network-and-inputs](positions/network-and-inputs.md), [`cities/mumbai/docs/scaling.md`](../cities/mumbai/docs/scaling.md) | the 10 % probe on the D15 host, priced by `arm_cost.py` |
