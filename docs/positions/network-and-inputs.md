...
**Updated:** 22 September 2026 (fifty-ninth session) · **Record read through:** §9.207 · **Written against family:** `F35`
...
- **The pass-through merge is measured and not applied** (§9.207, D15): `src/build/merge_pass_through_nodes.py` on the 846,699-link base network merges 37,122 of 377,443 nodes (33,923 more held by the 500 m cap, 349 by turn restrictions) and moves the median link 62.7 → 69.0 m at 101,306 km before and after; the user keeps the network exactly as converted. Nothing wires it into a build.
...
- **Two assumptions measured** (§9.140, #63): `E.s0.heavy_rail_detour_factor` 1.037 (sweep 1.0–1.1); `B.external.interaction_rate` 0.0900, derived.
...
- **The `networks/matsim/*` ODbL glob is narrowed on a content pass** (§9.159, #165): **110 of 111 rows confirmed ODbL**.
...
- **The 25 % heap is measured on the footpath network; the slope is NOT re-declared** (§9.169): arm 0's `gc.log` peaks at 26.2 GiB with no upward slope; the rule (15.6 + 87 × 0.25 = 37.4 GiB) holds 11 GiB over it.
...
- **40 of Mumbai's 43 unobtained sources sit on hosts that refuse this address** (§9.207, D17): MMMOCL, CR, WR, RDSO, MRVC time out or drop, the DES pages fail certificate trust — under Python, Windows TLS and a real browser alike; the capture needs a network the hosts admit. Two wait on the OGD platform's registered key (`OGD_API_KEY` in `.env`, D16): MMRDA's daily ridership of Metro 2A/7 and the Monorail (`ogd_mmrda_ridership_catalog_20260922`).
