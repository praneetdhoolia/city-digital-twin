...
- **Mumbai's transit vehicles carry evidenced capacities** (§9.206): `explicit_vehicle`; `build_transit_fleet.py` gives all 114,670 mapped vehicles a profile — 12-car EMU 1,168 + 3,816 (IRIMEE), Line 1 200 + 1,300, BEML 6-car 239 + 1,561, Line 3 399 + 2,601, Navi Mumbai 150 + 950, launches 80 and 100, bus 36 + 30 (standing assumed, swept) — not the mapper's defaults (Bus 70, Rail 400, Subway 300, Ferry 250, no standing) every earlier case ran on. The commuter crossings are not in the feed (#245).
...
- **The second city runs citywide plans through the harness against derived targets** (§9.201–§9.205, D13): 380 fields; the core is the notified MMR (2,538 leaves); 27.06 M persons for 2026; plans for the households the nested hash keeps at a 0.05 build fraction (1.35 M, `build_plans.py`); thirteen targets (`build_mode_targets.py`); the 0.1 % case is a check, not a reading: 140 KB an agent, 1.8 veh/h a lane (D14). See [`cities/mumbai/docs/README.md`](../../cities/mumbai/docs/README.md).
...
- **Two assumptions measured** (§9.140, #63): `E.s0.heavy_rail_detour_factor` 1.037 (sweep 1.0–1.1); `B.external.interaction_rate` 0.0900 derived from the commute share and the employed share.
...
- **Not one MATSim default decides this model unreviewed** (§9.164, #155): nine are registry fields at the framework's value; twelve accepted (`config/schema/matsim_defaults_accepted.json`).
