...
- **The network is rebuilt with its footpaths and every feed re-mapped once on it** (§9.167, #183): **181,892 → 368,230 links, 84,242 → 153,237 nodes**; pt2matsim drift on the remap: stop-link **100 %**, route sequences **82.3 %** (§3.5); signals, crossings (Clyde Street 204 → 203), dwells, run inputs, the count-station map (**197 → 202**) and the mode targets re-derived. Family **F34**.
- **A rebuild costs 4 minutes** (§9.167); the 1 % probe paces **40 s an iteration against 29.5 s** on the road graph (`20260912T065939_4it_1pct`); `RUN.machine.heap_floor_gib` 9.6 → 15.6 (`citysim.SharedModeNetworks`).
- **MATSim defaults**: `config/schema/matsim_defaults_accepted.json` holds **29** accepted, none deciding the transport system; undeclared **31 → 21** (§9.163, #155), **21 → 0** (§9.164, below).
- **Three declared fields reach the config off** — `B.mode.walk_feasible_km` 0.0, `B.mode.bike_feasible_km` 0.0, `C.crowding.seated_multiplier` 1.0 — declaring `inert_at` (§9.163); the five GitHub Actions are pinned to SHAs (`tests/unit/test_actions_pinned.py`).
- **Extent** derived, never typed (§9.35, #32): the five-LGA boundary plus `A.osm.harvest_margin_m` (5000 m); the CBD from the light rail stops plus `A.osm.buildings_margin_m` (3500 m); 4,086 km², 1,500 core SA1s (§1).
- **OSM harvest**: ten layers over tiles ≤ `A.osm.harvest_tile_deg` (0.4), merged by element id (§9.35); core SA1s without a road node 99 → 4; `data/raw/provenance_osm.json` (§9.141, #118).
...
- **Registry**: **559 fields** (`cities/newcastle/docs/reference/CONFIG_REFERENCE.md`; §9.167, #198, §9.177); `check_hardcoding.py --strict` at 0; every non-observed field carries a sweep, `held_fixed` or `derived_from`; three unobtained fields `value: null` (§15); no key carries a currency (§9.140, #62).
...
- **Toolchain**: `.tools/toolchain.json` pins Temurin JDK 25.0.4+7, pt2matsim 26.6 (MATSim 2027.0-2026w25, §9.73), Maven 3.9.9, the signals stack (201 jars); SUMO descoped (§9.74); `requirements.txt` pinned (§9.158).
- **Documents at `docs/`, five typed defaults declared** (§9.170, #198, #199): `city.docs()`; `B.activity.p_intermediate_stop_default`, `B.activity.intermediate_stop_shop_share`, `B.activity.distance_decay_target_default_km`, `A.road.speed_default_other_kmh`, `A.parking.capacity_default_other`; an unrecorded file dated from its mtime (`retrieved_basis`, #200).
...
- **Not one MATSim default decides this model unreviewed** (§9.164, #155): nine are registry fields at the framework's value — `RUN.qsim.traffic_dynamics` (`queue`, sweep `kinematicWaves`), `RUN.qsim.stuck_time_s` (10.0), `RUN.qsim.remove_stuck_vehicles` (false), `RUN.replanning.plan_selector_for_removal`, `RUN.routing.routing_randomness` (3.0), `RUN.travel_time.aggregator` (`optimistic`), `RUN.travel_time.getter` (`average`), `RUN.transit_router.additional_transfer_time_s` (0.0), `RUN.scoring.path_size_logit_beta` (1.0); twelve accepted.
...
- **The count-station map was orphaned by a network rebuild for 25 days** (§9.163, #82 closed): **0** of 195 rows in `count_station_links.csv` still named their road; regenerated, **197 rows, 0 unresolved**; `20260909T015217_300it_25pct` reads counts at mean **+16.30 %**, median **−1.1 %**, no station at zero.
...
- **A derived file's provenance is resolved from its lineage** (§9.151, #149; per output since §9.158): `retrieved` is the latest ancestor date, never a build time; **`source` 82 → 508 of 512, `retrieved` 59 → 443**; `params/C1_*`, `C5_calibration.json` and the 15 GTFS records stay blank at source.
...
- Circuity walk 1.6938, bike 1.5570, road 1.3276 on the CURRENT network (§9.142); walk speed 1.25 m/s (§9.33). Harvest defect: 87 core SA1s, 31,940 agents (5.2%) outside the network (§9.35). SAT:SUN 1.1473 (§9.61); `B.population.bike_available_rate` 0.493 (§9.78); VoT inside ±30% of EPV 2025 (§9.71).
- Crossings 110 and 204 a day against an assumed 30 (§9.90); ferry 107 and tram 252 weekday departures (§9.113).
- **Forty rows labelled CC-BY beside ODbL content are now ODbL** (§9.156): 30 schedules under `scenarios/matsim/` carry **357,893** OSM link references.
- **Lineage is resolved per output and the licence boundary is checked** (§9.158, #159): `OUTPUT_INPUTS` read statically; `share_alike_ancestor` yes **233** / no **279** / undetermined **0**; `check_lineage_licence()` in `tests/check_manifest.py` found a live breach on its first run (`params/C2_osm_defaults.json`).
- **The demand DOES carry OSM geometry** (§9.158): **3,000 of 3,000 sampled `dest_placement=poi` destinations within 5 m of an OSM POI or building** — `demand/plans/*` is ODbL; **35 licences moved**; `config.xml` gains no ancestor by referencing; `scenarios/matsim/_run_inputs_report.json` IS share-alike.
...
- A CRLF producer passes `check_manifest.py` locally and fails in CI (§9.142); every producer should write LF; until then a Windows rebuild is followed by normalisation.
...
- Storage capacity above flow capacity (§15). A GTFS-Realtime collector (§9.23). A service day from a route id (§9.113). Fitting the counts (#82).
...
- §9.151 — fallbacks declared; provenance from lineage
