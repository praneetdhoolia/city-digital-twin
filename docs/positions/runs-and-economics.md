...
- **A second arm is refused at preflight; a harness killed under a live JVM no longer aborts the run** (§9.169, §9.170): `refuse_concurrent_arm` in `src/run/run_matsim.py`; `reconcile_stale` tests `jvm_pid`; `CITYSIM_LAUNCH_STAMP` names a detached run after its task; an unjudged milestone is written to `_readings.jsonl`.
...
- **The routers pair landed in 27.39 h against a 30.0 h ceiling and a 29.3 h quote** (§9.176, `20260915T000704_250it_25pct`, overlay `f35_routers_mode_constant_25pct`, 48g: wall 98,590.8 s, `median_iteration_s` 362.34). Innovating iterations 340–450 s once a browser stopped sharing the CPU (§9.174), 405–561 s twice more while a game did (21:50–22:55 and from 00:50, `output/stopwatch.csv`); the post-cutoff tail 262–320 s; the it.50/100/150/200/250 dumps 600–867 s. The 30.0 h approval is SPENT.
- **Its harness died at iteration 34 and the JVM ran alone to the horizon** (§9.176, #225): `_progress.json` last written 05:11:22 on 15 Sep; no ceiling, stall or gate watcher from iteration ~35; the clean shutdown at 03:30:18 on 16 Sep wrote no record until `run.py --close-out` did.
...
- **No multi-hour run without explicit approval; approvals are spent on use** (§9.57, §9.62, §9.72): the 2 Sep directive was SPENT on `20260901T165115_300it_25pct` (§9.139), arm 0's 44 h on `20260912T202242_300it_25pct` (§9.169), the pair's 30.0 h on `20260915T000704_250it_25pct` (§9.174). **No approval stands.** **25 % runs only** (user, 1 Sep).
...
- `src/run/run_failure.py` names a throwable a run survived: on the pair's log the only throwable is Guice's `Unsupported class file major version 69` at line 481, and `from_log` returned it for a run that completed 250 iterations - `reconcile_stale()` would have recorded the result `failed` (§9.176, #225). The close-out reads the shutdown first; the reconcile does not yet.
- **The launch default is foreground** (§9.176, #225, D6 open): `--detach` exists and is not the default, and nothing refuses a foreground launch under a Claude session, so a session's end kills the harness.
