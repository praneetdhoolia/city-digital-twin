...
- **F33's first arm died on a heap nobody set** (§9.165): `aborted_20260910T222830_300it_25pct`, `OutOfMemoryError` at iteration 98 on 14g; a 4-iteration probe prices TIME, not HEAP.
...
- **The routers pair arm is running on a SPENT 30.0 h approval** (§9.174, `20260915T000704_250it_25pct`, overlay `f35_routers_mode_constant_25pct`, 48g): iteration 0 at 25.3 min; iterations 2–14 ran 427–568 s (mean 498 s) against the 422.0 s price (§9.172) - 505–568 s while a browser shared the CPU, 427 s once it stopped.
- **At 427–478 s the 250 iterations take 30–33 h, so the ceiling lands between iteration ~215 and the horizon** (§9.174); the user chose to let it run: a stop there is citable at its `reached_iteration`, not a result.
...
- **21.5 h uninterrupted and no stall** (§9.163, #66, `20260909T015217_300it_25pct`): one pace excursion (median 301.5 s over it.120–190) was a false alarm; 37.4 % of all machine hours bought no citable reading; an arm with no automatic stop is now refused (#169).
...
- **The store reclaimed 336.4 GiB** (§9.159, #164): raw 467.1 → 130.7 GiB; a 25 % arm's log is ~51 GiB by its gate (§9.156).
