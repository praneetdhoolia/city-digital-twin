...
- **The run viewer** `src/analyse/run_view.py` (§9.170–§9.175, §9.206): serves on EVERY run (`RUN.monitor.enabled` retired) and on any finished one (`--run <name>`); every run from one picker, each of a city's modes against the 10 % goal and 20 % stop bar; every transit route of the schedule the run drove, one layer and chip a transport mode (Mumbai 1,650 bus, 31 rail, 14 metro, 4 ferry); MapLibre GL 5.24.0, Overture buildings, Terrarium terrain; nothing from a city by name.
