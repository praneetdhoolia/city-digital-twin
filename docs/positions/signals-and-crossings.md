...
| Freight closures | **Derived** from the Cobbora Coal Project EA's March 2012 five-day survey: `A.crossings.freight_closures_per_day` 44 (Saint James Road) and 48 (Clyde Street) a day, 405 movements a weekday with the 313 scheduled (§9.167, #184); ARTC publishes no movement log, and the coal chain itself stays off the network, grade-separated since 2006 (§9.70) |
...
- Crossings: Saint James Road 110 scheduled closures per weekday and Clyde Street 203 (204 on the previous mapping, §3.5), against 30 assumed at both, plus 44 and 48 derived freight movements; 2,478 change events, peaked where the service is peaked (§9.90, §9.167). Mode 12 `freight_train` carries a derived target of 405 movements per weekday in `mode_targets_by_mode.csv` (§9.167).
- **Arm 0 loaded the closures, and the reader reads them like-for-like** (§9.169, `20260912T202242_300it_25pct`): the run's `crossing_change_events.xml` carried 266 merged closure spans (112 Saint James Road + 154 Clyde Street) for the 405 movements, and the reader's `freight_train` row reads **405 against 405** - it now adds the freight closures the run's own `_config.json` snapshot carried to the scheduled 313, where it had read 313 against 405 on every run, a 22.7 % bookkeeping shortfall (ninth report finding 4). Nothing else about signals changed this session.
...
- No arm-scale signal or crossing EFFECT is measured yet. A 1% probe verifies plumbing only, at about 0.3 vehicles per green (§9.76); the per-green discharge check at 25% reads 7.1–7.9 on the worst approach (§9.76). Every arm since F12 runs SCATS and the derived closures; the two results (§9.162, §9.169) carry both ON, and no paired arm has carried either off.

## What is open

- **Freight trains at the two crossings are derived from a 2012 survey, not counted at the gates** (#184, §9.167). `A.crossings.freight_closures_per_day` = 44 (Saint James Road) and 48 (Clyde Street) rest on the Cobbora EA's March 2012 five-day survey, with Clyde Street's freight share a stated assumption; the Newcastle Herald report of 10 June 2026 the eighth report's factor lane found is consistent with them. An ARTC train plan, the Lower Hunter Freight Corridor business case or a count at the gates would replace the derivation, and a change opens a family.

- **The signal-effect measurement at arm scale is still owed** (#73, CLOSED on the build scope): signals and priority are built, activated (§9.77) and adaptive (§9.88), and the controller and its guard are the evidence that closed it. What remains is not a build but a paired reading, and neither result (§9.162, §9.169) is one. Movement-level lanes stay data-gated at 16% coverage (§9.76). The issue's last comment predates §9.88; the SCATS build is not yet recorded on it.
