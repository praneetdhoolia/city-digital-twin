...
| [`reports/`](reports/README.md) | **Dated whole-project assessments** written by the `/project-report` skill: every file by area, the code redundancy / quality / efficiency pass, the simulator performance pass, the timeline from day 0, the milestones of every pull request, the issue ledger, CI, runs, data and documents, and the project placed against every comparable city twin and the research on what a decision simulator must contain, both researched afresh each time. Newest first; never overwritten |
