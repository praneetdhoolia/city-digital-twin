...
"""Render the dated /project-report HTML from the lanes' JSON deliverables.
    ...
    python .agents/skills/project-report/scripts/render_report.py <scratch> <out.html>
...
def modes_section(mt: dict) -> str:
    ...
    for m in modes:
        ...
        rows.append(r)
    labels = {"result": "newest completed result", "reading": "newest citable reading (see run status)", "target_provenance": "target provenance",
    ...
    lead = f'Result: `{mt.get("result_run")}`. Reading: `{mt.get("reading_run")}`. {mt.get("reader_caveat", "")}'
