...
| Analysis and calibration | `src/analyse/`, `src/calibrate/` | any path that presents a run whose `_run.json` does not say `ran_to_last_iteration` as a result, any figure quoted from a stopped arm past its `reached_iteration`, any deviation against an unscorable target, folding and ×1/fraction errors, hardcoded mode lists, duplicated event parsing |
