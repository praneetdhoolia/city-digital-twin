...
Loaded automatically into every Claude Code session in this repository.
...
2. **Plan, then get sign-off** before writing files.
3. **Implement** only after approval. Prefer a clear TODO to a speculative
   implementation.
...
- **Network access** is sandboxed to the project's data sources
  (`sandbox.network` in [`settings.json`](settings.json)). A new source means
  adding its domain there and a provenance record.
