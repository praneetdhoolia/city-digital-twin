...
DEFAULT_OUT = _city.city_docs('reference', 'CALIBRATION_REPORT.md')
CAL = _city.path('params/C5_calibration.json')


def load(tag):
    run_dir = _resolve_run(tag) if not os.path.isdir(tag) else tag
...
def main():
    ...
    w('*Generated by `src/calibrate/report.py` on %s from `_fit.json`. '
      'Regenerate it; do not edit it. Derived from the synthetic plans, which '
      'carry the OSM-derived network\'s share-alike ancestry (DECISIONS.md '
      '9.158): published under ODbL 1.0, not CC-BY 4.0.*\n\n'
