...
def _verdict_lines(rel, integ):
    """The verdict table - relaxation, accounting, telemetry - and the
    warning a run that has not settled carries."""
    L = []
    A = L.append
...
def _run_lines(r):
    """What was run, with the 1% storage-floor warning where it applies."""
    L = []
    A = L.append
    A('## What was run')
    A('')
...
def _relaxation_lines(rel, r):
    """Drift per mode after innovation was disabled; empty when none was measured."""
    L = []
    A = L.append
    if rel.get('drift_pp'):
...
def _accounting_lines(integ):
    """The conservation table and its notes; empty when there is no detail."""
    L = []
    A = L.append
...
def _fleet_lines(doc):
    """Peak simultaneous transit fleet by vehicle type; empty when none."""
    L = []
    A = L.append
...
def _next_lines(doc):
    """The commands that lead from this run to a reportable number."""
    L = []
    A = L.append
...
def markdown(doc):
    ...
    L.extend(_verdict_lines(rel, integ))
    L.extend(_run_lines(r))
    L.extend(_relaxation_lines(rel, r))
    L.extend(_accounting_lines(integ))
    L.extend(_fleet_lines(doc))
    L.extend(_next_lines(doc))
    return '\n'.join(L) + '\n'
